"""
International Trade Law Expert - Expert in expert ai for trade agreements, wto law, and international commerce regulations
"""
...
class InternationalTradeLawInput(BaseModel):
    """Input schema for international trade law queries."""
    query: str = Field(..., description="International Trade Law related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within international trade law")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class InternationalTradeLawOutput(BaseModel):
    """Output schema for international trade law analysis."""
    analysis: str = Field(..., description="Comprehensive international trade law analysis and insights")
...
class InternationalTradeLawAgent(BaseAgent):
    """
    Specialized AI agent for international trade law.
    Expert AI for trade agreements, WTO law, and international commerce regulations.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="International Trade Law Expert",
            description="""Expert AI for trade agreements, WTO law, and international commerce regulations.
            Specializes in WTO law, trade agreements, customs law, trade disputes, export controls.
            ...
            price_per_use=2.5,
            tags=["international-trade-law", "international trade law", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive international trade law analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return InternationalTradeLawInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return InternationalTradeLawOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute international trade law analysis using Claude AI."""
        try:
            # Validate input
            validated_input = InternationalTradeLawInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert International Trade Law Expert with deep knowledge in WTO law, trade agreements, customs law, trade disputes, export controls.

Your role is to:
1. Provide comprehensive analysis of international trade law queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: InternationalTradeLawInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: InternationalTradeLawInput
    ) -> InternationalTradeLawOutput:
        ...
        if not methodologies:
            methodologies = ["Standard international trade law analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "international trade law",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["international trade law"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in international trade law",
