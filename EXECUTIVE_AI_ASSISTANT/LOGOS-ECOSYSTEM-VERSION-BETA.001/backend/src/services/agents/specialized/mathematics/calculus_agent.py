"""
Calculus Specialist - Expert in expert ai for differential and integral calculus, multivariable calculus, and analysis
"""
...
class CalculusInput(BaseModel):
    """Input schema for calculus queries."""
    query: str = Field(..., description="Calculus related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within calculus")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class CalculusOutput(BaseModel):
    """Output schema for calculus analysis."""
    analysis: str = Field(..., description="Comprehensive calculus analysis and insights")
...
class CalculusAgent(BaseAgent):
    """
    Specialized AI agent for calculus.
    Expert AI for differential and integral calculus, multivariable calculus, and analysis.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Calculus Specialist",
            description="""Expert AI for differential and integral calculus, multivariable calculus, and analysis.
            Specializes in limits, derivatives, integrals, series, differential equations, vector calculus.
            ...
            price_per_use=1.5,
            tags=["calculus", "calculus", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive calculus analysis",
    ...
    def get_output_schema(self) -> Type[BaseModel]:
        ...
        return CalculusOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute calculus analysis using Claude AI."""
        try:
            # Validate input
            validated_input = CalculusInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Calculus Specialist with deep knowledge in limits, derivatives, integrals, series, differential equations, vector calculus.

Your role is to:
1. Provide comprehensive analysis of calculus queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: CalculusInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        ...
        if not methodologies:
            methodologies = ["Standard calculus analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "calculus",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["calculus"]
        }
