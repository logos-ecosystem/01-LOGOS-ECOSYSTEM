"""
Group Theory Specialist - Expert in expert ai for group theory, symmetry analysis, and algebraic structures
"""
...
class GroupTheoryInput(BaseModel):
    """Input schema for group theory queries."""
    query: str = Field(..., description="Group Theory related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within group theory")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class GroupTheoryOutput(BaseModel):
    """Output schema for group theory analysis."""
    analysis: str = Field(..., description="Comprehensive group theory analysis and insights")
...
class GroupTheoryAgent(BaseAgent):
    """
    Specialized AI agent for group theory.
    Expert AI for group theory, symmetry analysis, and algebraic structures.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Group Theory Specialist",
            description="""Expert AI for group theory, symmetry analysis, and algebraic structures.
            Specializes in finite groups, Lie groups, representation theory, group actions, symmetry.
            ...
            price_per_use=2.0,
            tags=["group-theory", "group theory", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive group theory analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return GroupTheoryInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return GroupTheoryOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute group theory analysis using Claude AI."""
        try:
            # Validate input
            validated_input = GroupTheoryInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Group Theory Specialist with deep knowledge in finite groups, Lie groups, representation theory, group actions, symmetry.

Your role is to:
1. Provide comprehensive analysis of group theory queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: GroupTheoryInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: GroupTheoryInput
    ) -> GroupTheoryOutput:
        ...
        if not methodologies:
            methodologies = ["Standard group theory analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "group theory",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["group theory"]
        }
