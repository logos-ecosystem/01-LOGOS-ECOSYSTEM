"""
Real Analysis Expert - Expert in expert ai for real number theory, limits, and mathematical analysis
"""
...
class RealAnalysisInput(BaseModel):
    """Input schema for real analysis queries."""
    query: str = Field(..., description="Real Analysis related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within real analysis")
...
class RealAnalysisOutput(BaseModel):
    """Output schema for real analysis analysis."""
    analysis: str = Field(..., description="Comprehensive real analysis analysis and insights")
...
class RealAnalysisAgent(BaseAgent):
    """
    Specialized AI agent for real analysis.
    Expert AI for real number theory, limits, and mathematical analysis.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Real Analysis Expert",
            description="""Expert AI for real number theory, limits, and mathematical analysis.
            Specializes in limits, continuity, differentiation, integration, metric spaces.
            ...
            price_per_use=1.5,
            tags=["real-analysis", "real analysis", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive real analysis analysis",
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute real analysis analysis using Claude AI."""
        try:
            # Validate input
            validated_input = RealAnalysisInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Real Analysis Expert with deep knowledge in limits, continuity, differentiation, integration, metric spaces.

Your role is to:
1. Provide comprehensive analysis of real analysis queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: RealAnalysisInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: RealAnalysisInput
    ) -> RealAnalysisOutput:
        ...
        if not methodologies:
            methodologies = ["Standard real analysis analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "real analysis",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["real analysis"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in real analysis",
