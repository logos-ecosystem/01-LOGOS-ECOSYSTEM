"""
Number Theory Specialist - Expert in expert ai for prime numbers, modular arithmetic, and algebraic number theory
"""
...
class NumberTheoryInput(BaseModel):
    """Input schema for number theory queries."""
    query: str = Field(..., description="Number Theory related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within number theory")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class NumberTheoryOutput(BaseModel):
    """Output schema for number theory analysis."""
    analysis: str = Field(..., description="Comprehensive number theory analysis and insights")
...
class NumberTheoryAgent(BaseAgent):
    """
    Specialized AI agent for number theory.
    Expert AI for prime numbers, modular arithmetic, and algebraic number theory.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Number Theory Specialist",
            description="""Expert AI for prime numbers, modular arithmetic, and algebraic number theory.
            Specializes in prime numbers, congruences, Diophantine equations, cryptographic applications.
            ...
            price_per_use=2.0,
            tags=["number-theory", "number theory", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return NumberTheoryInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return NumberTheoryOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute number theory analysis using Claude AI."""
        try:
            # Validate input
            validated_input = NumberTheoryInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Number Theory Specialist with deep knowledge in prime numbers, congruences, Diophantine equations, cryptographic applications.

Your role is to:
1. Provide comprehensive analysis of number theory queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: NumberTheoryInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: NumberTheoryInput
    ) -> NumberTheoryOutput:
        ...
        if not methodologies:
            methodologies = ["Standard number theory analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "number theory",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["number theory"]
        }
