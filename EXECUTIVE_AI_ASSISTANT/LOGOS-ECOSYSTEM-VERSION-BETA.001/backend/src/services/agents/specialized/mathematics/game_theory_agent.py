"""
Game Theory Specialist - Expert in expert ai for strategic decision-making, equilibria, and game analysis
"""
...
class GameTheoryInput(BaseModel):
    """Input schema for game theory queries."""
    query: str = Field(..., description="Game Theory related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within game theory")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class GameTheoryOutput(BaseModel):
    """Output schema for game theory analysis."""
    analysis: str = Field(..., description="Comprehensive game theory analysis and insights")
...
class GameTheoryAgent(BaseAgent):
    """
    Specialized AI agent for game theory.
    Expert AI for strategic decision-making, equilibria, and game analysis.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Game Theory Specialist",
            description="""Expert AI for strategic decision-making, equilibria, and game analysis.
            Specializes in Nash equilibrium, cooperative games, mechanism design, evolutionary game theory.
            ...
            price_per_use=1.5,
            tags=["game-theory", "game theory", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive game theory analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return GameTheoryInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return GameTheoryOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute game theory analysis using Claude AI."""
        try:
            # Validate input
            validated_input = GameTheoryInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Game Theory Specialist with deep knowledge in Nash equilibrium, cooperative games, mechanism design, evolutionary game theory.

Your role is to:
1. Provide comprehensive analysis of game theory queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: GameTheoryInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: GameTheoryInput
    ) -> GameTheoryOutput:
        ...
        if not methodologies:
            methodologies = ["Standard game theory analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "game theory",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["game theory"]
        }
