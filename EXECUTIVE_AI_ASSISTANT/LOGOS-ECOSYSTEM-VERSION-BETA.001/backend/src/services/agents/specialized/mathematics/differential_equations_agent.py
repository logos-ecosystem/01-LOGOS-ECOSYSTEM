"""
Differential Equations Expert - Expert in expert ai for ordinary and partial differential equations, dynamical systems
"""
...
class DifferentialEquationsInput(BaseModel):
    """Input schema for differential equations queries."""
    query: str = Field(..., description="Differential Equations related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within differential equations")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class DifferentialEquationsOutput(BaseModel):
    """Output schema for differential equations analysis."""
    analysis: str = Field(..., description="Comprehensive differential equations analysis and insights")
...
class DifferentialEquationsAgent(BaseAgent):
    """
    Specialized AI agent for differential equations.
    Expert AI for ordinary and partial differential equations, dynamical systems.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Differential Equations Expert",
            description="""Expert AI for ordinary and partial differential equations, dynamical systems.
            Specializes in ODEs, PDEs, boundary value problems, numerical methods, stability analysis.
            ...
            price_per_use=2.0,
            tags=["differential-equations", "differential equations", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive differential equations analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return DifferentialEquationsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return DifferentialEquationsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute differential equations analysis using Claude AI."""
        try:
            # Validate input
            validated_input = DifferentialEquationsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Differential Equations Expert with deep knowledge in ODEs, PDEs, boundary value problems, numerical methods, stability analysis.

Your role is to:
1. Provide comprehensive analysis of differential equations queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: DifferentialEquationsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: DifferentialEquationsInput
    ) -> DifferentialEquationsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard differential equations analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "differential equations",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["differential equations"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in differential equations",
