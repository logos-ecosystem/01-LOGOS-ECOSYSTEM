"""
Linear Algebra Expert - Expert in expert ai for vector spaces, matrices, linear transformations, and eigenvalues
"""
...
class LinearAlgebraInput(BaseModel):
    """Input schema for linear algebra queries."""
    query: str = Field(..., description="Linear Algebra related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within linear algebra")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class LinearAlgebraOutput(BaseModel):
    """Output schema for linear algebra analysis."""
    analysis: str = Field(..., description="Comprehensive linear algebra analysis and insights")
...
class LinearAlgebraAgent(BaseAgent):
    """
    Specialized AI agent for linear algebra.
    Expert AI for vector spaces, matrices, linear transformations, and eigenvalues.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Linear Algebra Expert",
            description="""Expert AI for vector spaces, matrices, linear transformations, and eigenvalues.
            Specializes in matrix operations, eigenvalues, vector spaces, linear transformations, applications.
            ...
            price_per_use=1.5,
            tags=["linear-algebra", "linear algebra", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive linear algebra analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return LinearAlgebraInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return LinearAlgebraOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute linear algebra analysis using Claude AI."""
        try:
            # Validate input
            validated_input = LinearAlgebraInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Linear Algebra Expert with deep knowledge in matrix operations, eigenvalues, vector spaces, linear transformations, applications.

Your role is to:
1. Provide comprehensive analysis of linear algebra queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: LinearAlgebraInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: LinearAlgebraInput
    ) -> LinearAlgebraOutput:
        ...
        if not methodologies:
            methodologies = ["Standard linear algebra analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "linear algebra",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["linear algebra"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in linear algebra",
