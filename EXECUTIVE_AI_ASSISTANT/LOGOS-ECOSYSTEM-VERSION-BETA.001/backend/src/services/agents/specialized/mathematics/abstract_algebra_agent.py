"""
Abstract Algebra Expert - Expert in expert ai for group theory, ring theory, field theory, and abstract algebraic structures
"""
...
class AbstractAlgebraInput(BaseModel):
    """Input schema for abstract algebra queries."""
    query: str = Field(..., description="Abstract Algebra related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within abstract algebra")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class AbstractAlgebraOutput(BaseModel):
    """Output schema for abstract algebra analysis."""
    analysis: str = Field(..., description="Comprehensive abstract algebra analysis and insights")
...
class AbstractAlgebraAgent(BaseAgent):
    """
    Specialized AI agent for abstract algebra.
    Expert AI for group theory, ring theory, field theory, and abstract algebraic structures.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Abstract Algebra Expert",
            description="""Expert AI for group theory, ring theory, field theory, and abstract algebraic structures.
            Specializes in groups, rings, fields, modules, Galois theory, representation theory.
            ...
            price_per_use=2.0,
            tags=["abstract-algebra", "abstract algebra", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive abstract algebra analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return AbstractAlgebraInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return AbstractAlgebraOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute abstract algebra analysis using Claude AI."""
        try:
            # Validate input
            validated_input = AbstractAlgebraInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Abstract Algebra Expert with deep knowledge in groups, rings, fields, modules, Galois theory, representation theory.

Your role is to:
1. Provide comprehensive analysis of abstract algebra queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: AbstractAlgebraInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: AbstractAlgebraInput
    ) -> AbstractAlgebraOutput:
        ...
        if not methodologies:
            methodologies = ["Standard abstract algebra analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "abstract algebra",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["abstract algebra"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in abstract algebra",
