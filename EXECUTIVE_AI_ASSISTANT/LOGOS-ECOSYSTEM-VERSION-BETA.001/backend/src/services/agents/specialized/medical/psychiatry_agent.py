"""
Psychiatry Specialist - Expert in expert ai for mental health disorders, psychological assessment, and psychiatric treatment
"""
...
class PsychiatryInput(BaseModel):
    """Input schema for psychiatry queries."""
    query: str = Field(..., description="Psychiatry related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within psychiatry")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class PsychiatryOutput(BaseModel):
    """Output schema for psychiatry analysis."""
    analysis: str = Field(..., description="Comprehensive psychiatry analysis and insights")
...
class PsychiatryAgent(BaseAgent):
    """
    Specialized AI agent for psychiatry.
    Expert AI for mental health disorders, psychological assessment, and psychiatric treatment.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Psychiatry Specialist",
            description="""Expert AI for mental health disorders, psychological assessment, and psychiatric treatment.
            Specializes in mood disorders, anxiety, psychosis, personality disorders, psychotherapy approaches.
            ...
            price_per_use=2.5,
            tags=["psychiatry", "psychiatry", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive psychiatry analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return PsychiatryInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return PsychiatryOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute psychiatry analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PsychiatryInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Psychiatry Specialist with deep knowledge in mood disorders, anxiety, psychosis, personality disorders, psychotherapy approaches.

Your role is to:
1. Provide comprehensive analysis of psychiatry queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PsychiatryInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: PsychiatryInput
    ) -> PsychiatryOutput:
        ...
        if not methodologies:
            methodologies = ["Standard psychiatry analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "psychiatry",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["psychiatry"]
        }
