"""
Neurosurgery Expert - Expert in expert ai for neurosurgical procedures, brain surgery, and spine surgery
"""
...
class NeurosurgeryInput(BaseModel):
    """Input schema for neurosurgery queries."""
    query: str = Field(..., description="Neurosurgery related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within neurosurgery")
...
class NeurosurgeryOutput(BaseModel):
    """Output schema for neurosurgery analysis."""
    analysis: str = Field(..., description="Comprehensive neurosurgery analysis and insights")
...
class NeurosurgeryAgent(BaseAgent):
    """
    Specialized AI agent for neurosurgery.
    Expert AI for neurosurgical procedures, brain surgery, and spine surgery.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Neurosurgery Expert",
            description="""Expert AI for neurosurgical procedures, brain surgery, and spine surgery.
            Specializes in brain surgery, spine surgery, tumor resection, vascular neurosurgery.
            ...
            price_per_use=3.0,
            tags=["neurosurgery", "neurosurgery", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive neurosurgery analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return NeurosurgeryInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return NeurosurgeryOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute neurosurgery analysis using Claude AI."""
        try:
            # Validate input
            validated_input = NeurosurgeryInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Neurosurgery Expert with deep knowledge in brain surgery, spine surgery, tumor resection, vascular neurosurgery.

Your role is to:
1. Provide comprehensive analysis of neurosurgery queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: NeurosurgeryInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: NeurosurgeryInput
    ) -> NeurosurgeryOutput:
        ...
        if not methodologies:
            methodologies = ["Standard neurosurgery analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "neurosurgery",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["neurosurgery"]
        }
