"""
Sports Medicine Specialist - Expert in expert ai for sports injuries, athletic performance, and rehabilitation
"""
...
class SportsMedicineInput(BaseModel):
    """Input schema for sports medicine queries."""
    query: str = Field(..., description="Sports Medicine related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within sports medicine")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class SportsMedicineOutput(BaseModel):
    """Output schema for sports medicine analysis."""
    analysis: str = Field(..., description="Comprehensive sports medicine analysis and insights")
...
class SportsMedicineAgent(BaseAgent):
    """
    Specialized AI agent for sports medicine.
    Expert AI for sports injuries, athletic performance, and rehabilitation.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Sports Medicine Specialist",
            description="""Expert AI for sports injuries, athletic performance, and rehabilitation.
            Specializes in sports injuries, performance optimization, rehabilitation, injury prevention.
            ...
            price_per_use=2.0,
            tags=["sports-medicine", "sports medicine", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive sports medicine analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return SportsMedicineInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return SportsMedicineOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute sports medicine analysis using Claude AI."""
        try:
            # Validate input
            validated_input = SportsMedicineInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Sports Medicine Specialist with deep knowledge in sports injuries, performance optimization, rehabilitation, injury prevention.

Your role is to:
1. Provide comprehensive analysis of sports medicine queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: SportsMedicineInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: SportsMedicineInput
    ) -> SportsMedicineOutput:
        ...
        if not methodologies:
            methodologies = ["Standard sports medicine analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "sports medicine",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["sports medicine"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in sports medicine",
