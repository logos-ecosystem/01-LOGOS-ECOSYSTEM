"""
Neuropharmacology Specialist - Expert in expert ai for brain drug interactions, neurotransmitters, and psychopharmacology
"""
...
class NeuropharmacologyAgentInput(BaseModel):
    """Input schema for neuropharmacology queries."""
    query: str = Field(..., description="Neuropharmacology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within neuropharmacology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class NeuropharmacologyAgentOutput(BaseModel):
    """Output schema for neuropharmacology analysis."""
    analysis: str = Field(..., description="Comprehensive neuropharmacology analysis and insights")
...
class NeuropharmacologyAgent(BaseAgent):
    """
    Specialized AI agent for neuropharmacology.
    Expert AI for brain drug interactions, neurotransmitters, and psychopharmacology.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Neuropharmacology Specialist",
            description="""Expert AI for brain drug interactions, neurotransmitters, and psychopharmacology.
            Specializes in drug mechanisms, neurotransmitter systems, psychiatric medications, drug development.
            ...
            price_per_use=3.0,
            tags=["neuropharmacology", "neuropharmacology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive neuropharmacology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return NeuropharmacologyAgentInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return NeuropharmacologyAgentOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute neuropharmacology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = NeuropharmacologyAgentInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Neuropharmacology Specialist with deep knowledge in drug mechanisms, neurotransmitter systems, psychiatric medications, drug development.

Your role is to:
1. Provide comprehensive analysis of neuropharmacology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: NeuropharmacologyAgentInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: NeuropharmacologyAgentInput
    ) -> NeuropharmacologyAgentOutput:
        ...
        if not methodologies:
            methodologies = ["Standard neuropharmacology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "neuropharmacology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["neuropharmacology"]
        }
