"""
Clinical Genetics Expert - Expert in expert ai for genetic disorders, inheritance patterns, and genetic counseling
"""
...
class ClinicalGeneticsInput(BaseModel):
    """Input schema for clinical genetics queries."""
    query: str = Field(..., description="Clinical Genetics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within clinical genetics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ClinicalGeneticsOutput(BaseModel):
    """Output schema for clinical genetics analysis."""
    analysis: str = Field(..., description="Comprehensive clinical genetics analysis and insights")
...
class ClinicalGeneticsAgent(BaseAgent):
    """
    Specialized AI agent for clinical genetics.
    Expert AI for genetic disorders, inheritance patterns, and genetic counseling.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Clinical Genetics Expert",
            description="""Expert AI for genetic disorders, inheritance patterns, and genetic counseling.
            Specializes in genetic testing, inheritance patterns, genetic counseling, chromosomal disorders.
            ...
            price_per_use=2.5,
            tags=["clinical-genetics", "clinical genetics", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive clinical genetics analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ClinicalGeneticsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ClinicalGeneticsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute clinical genetics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ClinicalGeneticsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Clinical Genetics Expert with deep knowledge in genetic testing, inheritance patterns, genetic counseling, chromosomal disorders.

Your role is to:
1. Provide comprehensive analysis of clinical genetics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ClinicalGeneticsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ClinicalGeneticsInput
    ) -> ClinicalGeneticsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard clinical genetics analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "clinical genetics",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["clinical genetics"]
        }
