"""
Veterinary Medicine Expert - Expert in expert ai for animal health, veterinary procedures, and comparative medicine
"""
...
class VeterinaryMedicineInput(BaseModel):
    """Input schema for veterinary medicine queries."""
    query: str = Field(..., description="Veterinary Medicine related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within veterinary medicine")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class VeterinaryMedicineOutput(BaseModel):
    """Output schema for veterinary medicine analysis."""
    analysis: str = Field(..., description="Comprehensive veterinary medicine analysis and insights")
...
class VeterinaryMedicineAgent(BaseAgent):
    """
    Specialized AI agent for veterinary medicine.
    Expert AI for animal health, veterinary procedures, and comparative medicine.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Veterinary Medicine Expert",
            description="""Expert AI for animal health, veterinary procedures, and comparative medicine.
            Specializes in small animal medicine, large animal medicine, surgery, zoonotic diseases.
            ...
            price_per_use=2.0,
            tags=["veterinary-medicine", "veterinary medicine", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive veterinary medicine analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return VeterinaryMedicineInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return VeterinaryMedicineOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute veterinary medicine analysis using Claude AI."""
        try:
            # Validate input
            validated_input = VeterinaryMedicineInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Veterinary Medicine Expert with deep knowledge in small animal medicine, large animal medicine, surgery, zoonotic diseases.

Your role is to:
1. Provide comprehensive analysis of veterinary medicine queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: VeterinaryMedicineInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: VeterinaryMedicineInput
    ) -> VeterinaryMedicineOutput:
        ...
        if not methodologies:
            methodologies = ["Standard veterinary medicine analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "veterinary medicine",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["veterinary medicine"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in veterinary medicine",
