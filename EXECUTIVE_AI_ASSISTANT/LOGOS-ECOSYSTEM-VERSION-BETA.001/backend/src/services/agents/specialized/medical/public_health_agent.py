"""
Public Health Expert - Expert in expert ai for population health, epidemiology, and health policy
"""
...
class PublicHealthInput(BaseModel):
    """Input schema for public health queries."""
    query: str = Field(..., description="Public Health related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within public health")
...
class PublicHealthOutput(BaseModel):
    """Output schema for public health analysis."""
    analysis: str = Field(..., description="Comprehensive public health analysis and insights")
...
class PublicHealthAgent(BaseAgent):
    """
    Specialized AI agent for public health.
    Expert AI for population health, epidemiology, and health policy.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Public Health Expert",
            description="""Expert AI for population health, epidemiology, and health policy.
            Specializes in disease prevention, health promotion, epidemiology, health systems, global health.
            ...
            price_per_use=2.0,
            tags=["public-health", "public health", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive public health analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return PublicHealthInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return PublicHealthOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute public health analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PublicHealthInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Public Health Expert with deep knowledge in disease prevention, health promotion, epidemiology, health systems, global health.

Your role is to:
1. Provide comprehensive analysis of public health queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PublicHealthInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: PublicHealthInput
    ) -> PublicHealthOutput:
        ...
        if not methodologies:
            methodologies = ["Standard public health analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "public health",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["public health"]
        }
