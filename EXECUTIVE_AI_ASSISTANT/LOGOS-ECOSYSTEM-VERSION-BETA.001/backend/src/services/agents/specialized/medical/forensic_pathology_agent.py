"""
Forensic Pathology Expert - Expert in expert ai for forensic medicine, cause of death analysis, and medicolegal issues
"""
...
class ForensicPathologyInput(BaseModel):
    """Input schema for forensic pathology queries."""
    query: str = Field(..., description="Forensic Pathology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within forensic pathology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ForensicPathologyOutput(BaseModel):
    """Output schema for forensic pathology analysis."""
    analysis: str = Field(..., description="Comprehensive forensic pathology analysis and insights")
...
class ForensicPathologyAgent(BaseAgent):
    """
    Specialized AI agent for forensic pathology.
    Expert AI for forensic medicine, cause of death analysis, and medicolegal issues.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Forensic Pathology Expert",
            description="""Expert AI for forensic medicine, cause of death analysis, and medicolegal issues.
            Specializes in autopsy, cause of death, forensic toxicology, trauma analysis, medicolegal aspects.
            ...
            price_per_use=3.0,
            tags=["forensic-pathology", "forensic pathology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive forensic pathology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ForensicPathologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ForensicPathologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute forensic pathology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ForensicPathologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Forensic Pathology Expert with deep knowledge in autopsy, cause of death, forensic toxicology, trauma analysis, medicolegal aspects.

Your role is to:
1. Provide comprehensive analysis of forensic pathology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ForensicPathologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ForensicPathologyInput
    ) -> ForensicPathologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard forensic pathology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "forensic pathology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["forensic pathology"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in forensic pathology",
