"""
Healthcare Operations Specialist - Expert in expert ai for healthcare management, operations optimization, and quality improvement
"""
...
class HealthcareOperationsInput(BaseModel):
    """Input schema for healthcare operations queries."""
    query: str = Field(..., description="Healthcare Operations related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within healthcare operations")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class HealthcareOperationsOutput(BaseModel):
    """Output schema for healthcare operations analysis."""
    analysis: str = Field(..., description="Comprehensive healthcare operations analysis and insights")
...
class HealthcareOperationsAgent(BaseAgent):
    """
    Specialized AI agent for healthcare operations.
    Expert AI for healthcare management, operations optimization, and quality improvement.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Healthcare Operations Specialist",
            description="""Expert AI for healthcare management, operations optimization, and quality improvement.
            Specializes in healthcare management, quality improvement, patient flow, resource optimization.
            ...
            price_per_use=2.0,
            tags=["healthcare-operations", "healthcare operations", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return HealthcareOperationsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return HealthcareOperationsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute healthcare operations analysis using Claude AI."""
        try:
            # Validate input
            validated_input = HealthcareOperationsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Healthcare Operations Specialist with deep knowledge in healthcare management, quality improvement, patient flow, resource optimization.

Your role is to:
1. Provide comprehensive analysis of healthcare operations queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: HealthcareOperationsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: HealthcareOperationsInput
    ) -> HealthcareOperationsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard healthcare operations analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "healthcare operations",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["healthcare operations"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in healthcare operations",
