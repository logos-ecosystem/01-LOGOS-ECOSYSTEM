"""
Ophthalmology Specialist - Expert in expert ai for eye diseases, vision care, and ophthalmic procedures
"""
...
class OphthalmologyInput(BaseModel):
    """Input schema for ophthalmology queries."""
    query: str = Field(..., description="Ophthalmology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within ophthalmology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class OphthalmologyOutput(BaseModel):
    """Output schema for ophthalmology analysis."""
    analysis: str = Field(..., description="Comprehensive ophthalmology analysis and insights")
...
class OphthalmologyAgent(BaseAgent):
    """
    Specialized AI agent for ophthalmology.
    Expert AI for eye diseases, vision care, and ophthalmic procedures.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Ophthalmology Specialist",
            description="""Expert AI for eye diseases, vision care, and ophthalmic procedures.
            Specializes in cataract, glaucoma, retinal diseases, refractive surgery, pediatric ophthalmology.
            ...
            price_per_use=2.5,
            tags=["ophthalmology", "ophthalmology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive ophthalmology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return OphthalmologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return OphthalmologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute ophthalmology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = OphthalmologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Ophthalmology Specialist with deep knowledge in cataract, glaucoma, retinal diseases, refractive surgery, pediatric ophthalmology.

Your role is to:
1. Provide comprehensive analysis of ophthalmology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: OphthalmologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: OphthalmologyInput
    ) -> OphthalmologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard ophthalmology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "ophthalmology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["ophthalmology"]
        }
