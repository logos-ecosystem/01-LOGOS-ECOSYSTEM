"""
Pediatrics Expert - Expert in expert ai for child health, pediatric diseases, and developmental medicine
"""
...
class PediatricsInput(BaseModel):
    """Input schema for pediatrics queries."""
    query: str = Field(..., description="Pediatrics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within pediatrics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class PediatricsOutput(BaseModel):
    """Output schema for pediatrics analysis."""
    analysis: str = Field(..., description="Comprehensive pediatrics analysis and insights")
...
class PediatricsAgent(BaseAgent):
    """
    Specialized AI agent for pediatrics.
    Expert AI for child health, pediatric diseases, and developmental medicine.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Pediatrics Expert",
            description="""Expert AI for child health, pediatric diseases, and developmental medicine.
            Specializes in child development, pediatric diseases, vaccination, growth disorders, behavioral pediatrics.
            ...
            price_per_use=2.5,
            tags=["pediatrics", "pediatrics", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive pediatrics analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return PediatricsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return PediatricsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute pediatrics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PediatricsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Pediatrics Expert with deep knowledge in child development, pediatric diseases, vaccination, growth disorders, behavioral pediatrics.

Your role is to:
1. Provide comprehensive analysis of pediatrics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PediatricsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: PediatricsInput
    ) -> PediatricsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard pediatrics analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "pediatrics",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["pediatrics"]
        }
