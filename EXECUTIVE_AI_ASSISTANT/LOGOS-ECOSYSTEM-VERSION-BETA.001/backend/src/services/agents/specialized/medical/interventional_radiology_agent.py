"""
Interventional Radiology Expert - Expert in expert ai for minimally invasive procedures, imaging guidance, and vascular interventions
"""
...
class InterventionalRadiologyInput(BaseModel):
    """Input schema for interventional radiology queries."""
    query: str = Field(..., description="Interventional Radiology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within interventional radiology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class InterventionalRadiologyOutput(BaseModel):
    """Output schema for interventional radiology analysis."""
    analysis: str = Field(..., description="Comprehensive interventional radiology analysis and insights")
...
class InterventionalRadiologyAgent(BaseAgent):
    """
    Specialized AI agent for interventional radiology.
    Expert AI for minimally invasive procedures, imaging guidance, and vascular interventions.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Interventional Radiology Expert",
            description="""Expert AI for minimally invasive procedures, imaging guidance, and vascular interventions.
            Specializes in vascular interventions, tumor ablation, embolization, image-guided procedures.
            ...
            price_per_use=3.0,
            tags=["interventional-radiology", "interventional radiology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive interventional radiology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return InterventionalRadiologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return InterventionalRadiologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute interventional radiology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = InterventionalRadiologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Interventional Radiology Expert with deep knowledge in vascular interventions, tumor ablation, embolization, image-guided procedures.

Your role is to:
1. Provide comprehensive analysis of interventional radiology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: InterventionalRadiologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: InterventionalRadiologyInput
    ) -> InterventionalRadiologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard interventional radiology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "interventional radiology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["interventional radiology"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in interventional radiology",
