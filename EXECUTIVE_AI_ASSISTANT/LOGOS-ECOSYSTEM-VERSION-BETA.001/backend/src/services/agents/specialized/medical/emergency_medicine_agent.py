"""
Emergency Medicine Specialist - Expert in expert ai for emergency care, trauma management, and critical care medicine
"""
...
class EmergencyMedicineInput(BaseModel):
    """Input schema for emergency medicine queries."""
    query: str = Field(..., description="Emergency Medicine related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within emergency medicine")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class EmergencyMedicineOutput(BaseModel):
    """Output schema for emergency medicine analysis."""
    analysis: str = Field(..., description="Comprehensive emergency medicine analysis and insights")
...
class EmergencyMedicineAgent(BaseAgent):
    """
    Specialized AI agent for emergency medicine.
    Expert AI for emergency care, trauma management, and critical care medicine.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Emergency Medicine Specialist",
            description="""Expert AI for emergency care, trauma management, and critical care medicine.
            Specializes in trauma assessment, resuscitation, emergency procedures, triage, acute care.
            ...
            price_per_use=3.0,
            tags=["emergency-medicine", "emergency medicine", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive emergency medicine analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return EmergencyMedicineInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return EmergencyMedicineOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute emergency medicine analysis using Claude AI."""
        try:
            # Validate input
            validated_input = EmergencyMedicineInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Emergency Medicine Specialist with deep knowledge in trauma assessment, resuscitation, emergency procedures, triage, acute care.

Your role is to:
1. Provide comprehensive analysis of emergency medicine queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: EmergencyMedicineInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: EmergencyMedicineInput
    ) -> EmergencyMedicineOutput:
        ...
        if not methodologies:
            methodologies = ["Standard emergency medicine analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "emergency medicine",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["emergency medicine"]
        }
