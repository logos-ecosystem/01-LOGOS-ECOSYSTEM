"""
Genomics Expert - Expert in expert ai for genomic analysis, gene expression, and bioinformatics
"""
...
class GenomicsAgentInput(BaseModel):
    """Input schema for genomics queries."""
    query: str = Field(..., description="Genomics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within genomics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class GenomicsAgentOutput(BaseModel):
    """Output schema for genomics analysis."""
    analysis: str = Field(..., description="Comprehensive genomics analysis and insights")
...
class GenomicsAgent(BaseAgent):
    """
    Specialized AI agent for genomics.
    Expert AI for genomic analysis, gene expression, and bioinformatics.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Genomics Expert",
            description="""Expert AI for genomic analysis, gene expression, and bioinformatics.
            Specializes in genome sequencing, gene expression analysis, bioinformatics, genetic engineering.
            ...
            price_per_use=3.0,
            tags=["genomics", "genomics", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return GenomicsAgentInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return GenomicsAgentOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute genomics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = GenomicsAgentInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Genomics Expert with deep knowledge in genome sequencing, gene expression analysis, bioinformatics, genetic engineering.

Your role is to:
1. Provide comprehensive analysis of genomics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: GenomicsAgentInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: GenomicsAgentInput
    ) -> GenomicsAgentOutput:
        ...
        if not methodologies:
            methodologies = ["Standard genomics analysis", "Best practices evaluation", "Evidence-based approach"]
        ...
        technical_details = {
            ...
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["genomics"]
        }
