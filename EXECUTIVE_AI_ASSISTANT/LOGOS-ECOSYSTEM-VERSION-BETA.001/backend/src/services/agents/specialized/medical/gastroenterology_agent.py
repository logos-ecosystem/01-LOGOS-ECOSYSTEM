"""
Gastroenterology Specialist - Expert in expert ai for digestive system disorders, gi procedures, and hepatology
"""
...
class GastroenterologyInput(BaseModel):
    """Input schema for gastroenterology queries."""
    query: str = Field(..., description="Gastroenterology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within gastroenterology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class GastroenterologyOutput(BaseModel):
    """Output schema for gastroenterology analysis."""
    analysis: str = Field(..., description="Comprehensive gastroenterology analysis and insights")
...
class GastroenterologyAgent(BaseAgent):
    """
    Specialized AI agent for gastroenterology.
    Expert AI for digestive system disorders, GI procedures, and hepatology.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Gastroenterology Specialist",
            description="""Expert AI for digestive system disorders, GI procedures, and hepatology.
            Specializes in GI disorders, liver diseases, endoscopy, inflammatory bowel disease, hepatology.
            ...
            price_per_use=2.5,
            tags=["gastroenterology", "gastroenterology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive gastroenterology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return GastroenterologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return GastroenterologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute gastroenterology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = GastroenterologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Gastroenterology Specialist with deep knowledge in GI disorders, liver diseases, endoscopy, inflammatory bowel disease, hepatology.

Your role is to:
1. Provide comprehensive analysis of gastroenterology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: GastroenterologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: GastroenterologyInput
    ) -> GastroenterologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard gastroenterology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "gastroenterology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["gastroenterology"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in gastroenterology",
