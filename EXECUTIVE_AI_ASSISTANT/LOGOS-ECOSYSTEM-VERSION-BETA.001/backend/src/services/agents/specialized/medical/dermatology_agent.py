"""
Dermatology Specialist - Expert in expert ai for skin conditions, dermatological treatments, and skin health
"""
...
class DermatologyInput(BaseModel):
    """Input schema for dermatology queries."""
    query: str = Field(..., description="Dermatology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within dermatology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class DermatologyOutput(BaseModel):
    """Output schema for dermatology analysis."""
    analysis: str = Field(..., description="Comprehensive dermatology analysis and insights")
...
class DermatologyAgent(BaseAgent):
    """
    Specialized AI agent for dermatology.
    Expert AI for skin conditions, dermatological treatments, and skin health.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Dermatology Specialist",
            description="""Expert AI for skin conditions, dermatological treatments, and skin health.
            Specializes in skin disorders, dermatological treatments, cosmetic dermatology, skin cancer.
            ...
            price_per_use=2.5,
            tags=["dermatology", "dermatology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive dermatology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return DermatologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return DermatologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute dermatology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = DermatologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Dermatology Specialist with deep knowledge in skin disorders, dermatological treatments, cosmetic dermatology, skin cancer.

Your role is to:
1. Provide comprehensive analysis of dermatology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: DermatologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: DermatologyInput
    ) -> DermatologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard dermatology analysis", "Best practices evaluation", "Evidence-based approach"]
        ...
        technical_details = {
            ...
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["dermatology"]
        }
