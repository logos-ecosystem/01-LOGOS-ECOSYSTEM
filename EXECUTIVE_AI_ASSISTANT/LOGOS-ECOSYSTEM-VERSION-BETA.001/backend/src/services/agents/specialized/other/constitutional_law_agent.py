"""
Constitutional Law Specialist - Expert in expert ai for constitutional interpretation, civil rights, and governmental powers
"""
...
class ConstitutionalLawInput(BaseModel):
    """Input schema for constitutional law queries."""
    query: str = Field(..., description="Constitutional Law related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within constitutional law")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ConstitutionalLawOutput(BaseModel):
    """Output schema for constitutional law analysis."""
    analysis: str = Field(..., description="Comprehensive constitutional law analysis and insights")
...
class ConstitutionalLawAgent(BaseAgent):
    """
    Specialized AI agent for constitutional law.
    Expert AI for constitutional interpretation, civil rights, and governmental powers.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Constitutional Law Specialist",
            description="""Expert AI for constitutional interpretation, civil rights, and governmental powers.
            Specializes in constitutional interpretation, civil rights, separation of powers, judicial review.
            ...
            price_per_use=2.5,
            tags=["constitutional-law", "constitutional law", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive constitutional law analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ConstitutionalLawInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ConstitutionalLawOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute constitutional law analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ConstitutionalLawInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Constitutional Law Specialist with deep knowledge in constitutional interpretation, civil rights, separation of powers, judicial review.

Your role is to:
1. Provide comprehensive analysis of constitutional law queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ConstitutionalLawInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ConstitutionalLawInput
    ) -> ConstitutionalLawOutput:
        ...
        if not methodologies:
            methodologies = ["Standard constitutional law analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "constitutional law",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["constitutional law"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in constitutional law",
