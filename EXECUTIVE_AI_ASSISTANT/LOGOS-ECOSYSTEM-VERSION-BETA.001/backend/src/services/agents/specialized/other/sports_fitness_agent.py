"""
Sports & Fitness Expert - Expert in expert ai for exercise science, fitness training, and sports performance
"""
...
class SportsFitnessInput(BaseModel):
    """Input schema for sports fitness queries."""
    query: str = Field(..., description="Sports Fitness related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within sports fitness")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class SportsFitnessOutput(BaseModel):
    """Output schema for sports fitness analysis."""
    analysis: str = Field(..., description="Comprehensive sports fitness analysis and insights")
...
class SportsFitnessAgent(BaseAgent):
    """
    Specialized AI agent for sports fitness.
    Expert AI for exercise science, fitness training, and sports performance.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Sports & Fitness Expert",
            description="""Expert AI for exercise science, fitness training, and sports performance.
            Specializes in exercise physiology, training programs, nutrition, injury prevention, performance.
            ...
            price_per_use=1.5,
            tags=["sports-fitness", "sports fitness", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive sports fitness analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return SportsFitnessInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return SportsFitnessOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute sports fitness analysis using Claude AI."""
        try:
            # Validate input
            validated_input = SportsFitnessInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Sports & Fitness Expert with deep knowledge in exercise physiology, training programs, nutrition, injury prevention, performance.

Your role is to:
1. Provide comprehensive analysis of sports fitness queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: SportsFitnessInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: SportsFitnessInput
    ) -> SportsFitnessOutput:
        ...
        if not methodologies:
            methodologies = ["Standard sports fitness analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "sports fitness",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["sports fitness"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in sports fitness",
