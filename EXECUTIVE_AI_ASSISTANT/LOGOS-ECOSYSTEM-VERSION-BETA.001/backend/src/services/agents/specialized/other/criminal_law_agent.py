"""
Criminal Law Expert - Expert in expert ai for criminal justice, penal codes, and criminal procedure
"""
...
class CriminalLawInput(BaseModel):
    """Input schema for criminal law queries."""
    query: str = Field(..., description="Criminal Law related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within criminal law")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class CriminalLawOutput(BaseModel):
    """Output schema for criminal law analysis."""
    analysis: str = Field(..., description="Comprehensive criminal law analysis and insights")
...
class CriminalLawAgent(BaseAgent):
    """
    Specialized AI agent for criminal law.
    Expert AI for criminal justice, penal codes, and criminal procedure.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Criminal Law Expert",
            description="""Expert AI for criminal justice, penal codes, and criminal procedure.
            Specializes in criminal procedure, evidence law, sentencing, criminal defense, prosecution.
            ...
            price_per_use=2.5,
            tags=["criminal-law", "criminal law", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return CriminalLawInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return CriminalLawOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute criminal law analysis using Claude AI."""
        try:
            # Validate input
            validated_input = CriminalLawInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Criminal Law Expert with deep knowledge in criminal procedure, evidence law, sentencing, criminal defense, prosecution.

Your role is to:
1. Provide comprehensive analysis of criminal law queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: CriminalLawInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: CriminalLawInput
    ) -> CriminalLawOutput:
        ...
        if not methodologies:
            methodologies = ["Standard criminal law analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "criminal law",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["criminal law"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in criminal law",
