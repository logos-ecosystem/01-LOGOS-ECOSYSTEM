"""
Urban Planning Specialist - Expert in expert ai for city planning, zoning, and sustainable urban development
"""
...
class UrbanPlanningInput(BaseModel):
    """Input schema for urban planning queries."""
    query: str = Field(..., description="Urban Planning related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within urban planning")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class UrbanPlanningOutput(BaseModel):
    """Output schema for urban planning analysis."""
    analysis: str = Field(..., description="Comprehensive urban planning analysis and insights")
...
class UrbanPlanningAgent(BaseAgent):
    """
    Specialized AI agent for urban planning.
    Expert AI for city planning, zoning, and sustainable urban development.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Urban Planning Specialist",
            description="""Expert AI for city planning, zoning, and sustainable urban development.
            Specializes in land use planning, transportation planning, sustainable development, zoning, urban design.
            ...
            price_per_use=2.0,
            tags=["urban-planning", "urban planning", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return UrbanPlanningInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return UrbanPlanningOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute urban planning analysis using Claude AI."""
        try:
            # Validate input
            validated_input = UrbanPlanningInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Urban Planning Specialist with deep knowledge in land use planning, transportation planning, sustainable development, zoning, urban design.

Your role is to:
1. Provide comprehensive analysis of urban planning queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: UrbanPlanningInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: UrbanPlanningInput
    ) -> UrbanPlanningOutput:
        ...
        if not methodologies:
            methodologies = ["Standard urban planning analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "urban planning",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["urban planning"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in urban planning",
