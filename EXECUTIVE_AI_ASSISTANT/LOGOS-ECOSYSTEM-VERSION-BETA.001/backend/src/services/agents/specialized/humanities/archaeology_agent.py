"""
Archaeology Specialist - Expert in expert ai for archaeological methods, ancient civilizations, and material culture
"""
...
class ArchaeologyInput(BaseModel):
    """Input schema for archaeology queries."""
    query: str = Field(..., description="Archaeology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within archaeology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ArchaeologyOutput(BaseModel):
    """Output schema for archaeology analysis."""
    analysis: str = Field(..., description="Comprehensive archaeology analysis and insights")
...
class ArchaeologyAgent(BaseAgent):
    """
    Specialized AI agent for archaeology.
    Expert AI for archaeological methods, ancient civilizations, and material culture.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Archaeology Specialist",
            description="""Expert AI for archaeological methods, ancient civilizations, and material culture.
            Specializes in excavation methods, artifact analysis, dating techniques, ancient civilizations.
            ...
            price_per_use=1.5,
            tags=["archaeology", "archaeology", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ArchaeologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ArchaeologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute archaeology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ArchaeologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Archaeology Specialist with deep knowledge in excavation methods, artifact analysis, dating techniques, ancient civilizations.

Your role is to:
1. Provide comprehensive analysis of archaeology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ArchaeologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ArchaeologyInput
    ) -> ArchaeologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard archaeology analysis", "Best practices evaluation", "Evidence-based approach"]
        ...
        technical_details = {
            ...
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["archaeology"]
        }
