"""
Arts & Culture Expert - Expert in expert ai for cultural studies, art history, and creative arts
"""
...
class ArtsCultureInput(BaseModel):
    """Input schema for arts culture queries."""
    query: str = Field(..., description="Arts Culture related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within arts culture")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ArtsCultureOutput(BaseModel):
    """Output schema for arts culture analysis."""
    analysis: str = Field(..., description="Comprehensive arts culture analysis and insights")
...
class ArtsCultureAgent(BaseAgent):
    """
    Specialized AI agent for arts culture.
    Expert AI for cultural studies, art history, and creative arts.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Arts & Culture Expert",
            description="""Expert AI for cultural studies, art history, and creative arts.
            Specializes in art history, cultural studies, contemporary art, art criticism, cultural theory.
            ...
            price_per_use=1.5,
            tags=["arts-culture", "arts culture", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive arts culture analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ArtsCultureInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ArtsCultureOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute arts culture analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ArtsCultureInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Arts & Culture Expert with deep knowledge in art history, cultural studies, contemporary art, art criticism, cultural theory.

Your role is to:
1. Provide comprehensive analysis of arts culture queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ArtsCultureInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ArtsCultureInput
    ) -> ArtsCultureOutput:
        ...
        if not methodologies:
            methodologies = ["Standard arts culture analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "arts culture",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["arts culture"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in arts culture",
