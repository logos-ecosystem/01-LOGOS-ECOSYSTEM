"""
Anthropology Expert - Expert in expert ai for cultural anthropology, human evolution, and ethnographic studies
"""
...
class AnthropologyInput(BaseModel):
    """Input schema for anthropology queries."""
    query: str = Field(..., description="Anthropology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within anthropology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class AnthropologyOutput(BaseModel):
    """Output schema for anthropology analysis."""
    analysis: str = Field(..., description="Comprehensive anthropology analysis and insights")
...
class AnthropologyAgent(BaseAgent):
    """
    Specialized AI agent for anthropology.
    Expert AI for cultural anthropology, human evolution, and ethnographic studies.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Anthropology Expert",
            description="""Expert AI for cultural anthropology, human evolution, and ethnographic studies.
            Specializes in cultural anthropology, biological anthropology, archaeology, linguistic anthropology.
            Provides detailed analysis, expert insights, and practical recommendations.""",
            category=AgentCategory.ACADEMIC_RESEARCH,
            version="1.0.0",
            author="LOGOS AI Team",
            pricing_model=PricingModel.PER_USE,
            price_per_use=1.5,
            tags=["anthropology", "anthropology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive anthropology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return AnthropologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return AnthropologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute anthropology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = AnthropologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Anthropology Expert with deep knowledge in cultural anthropology, biological anthropology, archaeology, linguistic anthropology.

Your role is to:
1. Provide comprehensive analysis of anthropology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: AnthropologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: AnthropologyInput
    ) -> AnthropologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard anthropology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "anthropology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["anthropology"]
        }
