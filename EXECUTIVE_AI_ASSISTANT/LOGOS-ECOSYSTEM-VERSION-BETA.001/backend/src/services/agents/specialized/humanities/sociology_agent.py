"""
Sociology Specialist - Expert in expert ai for social structures, social theory, and sociological research
"""
...
class SociologyInput(BaseModel):
    """Input schema for sociology queries."""
    query: str = Field(..., description="Sociology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within sociology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class SociologyOutput(BaseModel):
    """Output schema for sociology analysis."""
    analysis: str = Field(..., description="Comprehensive sociology analysis and insights")
...
class SociologyAgent(BaseAgent):
    """
    Specialized AI agent for sociology.
    Expert AI for social structures, social theory, and sociological research.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Sociology Specialist",
            description="""Expert AI for social structures, social theory, and sociological research.
            Specializes in social theory, research methods, social inequality, organizations, social movements.
            ...
            price_per_use=1.5,
            tags=["sociology", "sociology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive sociology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return SociologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return SociologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute sociology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = SociologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Sociology Specialist with deep knowledge in social theory, research methods, social inequality, organizations, social movements.

Your role is to:
1. Provide comprehensive analysis of sociology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: SociologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: SociologyInput
    ) -> SociologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard sociology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "sociology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["sociology"]
        }
