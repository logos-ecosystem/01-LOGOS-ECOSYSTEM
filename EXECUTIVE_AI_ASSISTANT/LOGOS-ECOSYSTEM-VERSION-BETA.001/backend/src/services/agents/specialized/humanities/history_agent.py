"""
History Expert - Expert in expert ai for historical analysis, historiography, and historical research
"""
...
class HistoryInput(BaseModel):
    """Input schema for history queries."""
    query: str = Field(..., description="History related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within history")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class HistoryOutput(BaseModel):
    """Output schema for history analysis."""
    analysis: str = Field(..., description="Comprehensive history analysis and insights")
...
class HistoryAgent(BaseAgent):
    """
    Specialized AI agent for history.
    Expert AI for historical analysis, historiography, and historical research.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="History Expert",
            description="""Expert AI for historical analysis, historiography, and historical research.
            Specializes in world history, historiography, historical methods, primary sources, historical interpretation.
            ...
            price_per_use=1.5,
            tags=["history", "history", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute history analysis using Claude AI."""
        try:
            # Validate input
            validated_input = HistoryInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert History Expert with deep knowledge in world history, historiography, historical methods, primary sources, historical interpretation.

Your role is to:
1. Provide comprehensive analysis of history queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: HistoryInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        ...
        if not methodologies:
            methodologies = ["Standard history analysis", "Best practices evaluation", "Evidence-based approach"]
        ...
        technical_details = {
            ...
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["history"]
        }
