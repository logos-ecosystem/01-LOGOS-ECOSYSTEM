"""
Linguistics Specialist - Expert in expert ai for language structure, phonetics, and linguistic theory
"""
...
class LinguisticsInput(BaseModel):
    """Input schema for linguistics queries."""
    query: str = Field(..., description="Linguistics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within linguistics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class LinguisticsOutput(BaseModel):
    """Output schema for linguistics analysis."""
    analysis: str = Field(..., description="Comprehensive linguistics analysis and insights")
...
class LinguisticsAgent(BaseAgent):
    """
    Specialized AI agent for linguistics.
    Expert AI for language structure, phonetics, and linguistic theory.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Linguistics Specialist",
            description="""Expert AI for language structure, phonetics, and linguistic theory.
            Specializes in phonetics, syntax, semantics, sociolinguistics, language acquisition.
            ...
            price_per_use=1.5,
            tags=["linguistics", "linguistics", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive linguistics analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return LinguisticsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return LinguisticsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute linguistics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = LinguisticsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Linguistics Specialist with deep knowledge in phonetics, syntax, semantics, sociolinguistics, language acquisition.

Your role is to:
1. Provide comprehensive analysis of linguistics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: LinguisticsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: LinguisticsInput
    ) -> LinguisticsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard linguistics analysis", "Best practices evaluation", "Evidence-based approach"]
        ...
        technical_details = {
            ...
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["linguistics"]
        }
