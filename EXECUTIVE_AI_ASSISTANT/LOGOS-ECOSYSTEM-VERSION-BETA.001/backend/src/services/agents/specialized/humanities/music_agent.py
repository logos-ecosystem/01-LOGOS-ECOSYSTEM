"""
Music Theory Expert - Expert in expert ai for music theory, composition, and musicology
"""
...
class MusicInput(BaseModel):
    """Input schema for music queries."""
    query: str = Field(..., description="Music related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within music")
...
class MusicOutput(BaseModel):
    """Output schema for music analysis."""
    analysis: str = Field(..., description="Comprehensive music analysis and insights")
...
class MusicAgent(BaseAgent):
    """
    Specialized AI agent for music.
    Expert AI for music theory, composition, and musicology.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Music Theory Expert",
            description="""Expert AI for music theory, composition, and musicology.
            Specializes in music theory, composition, harmony, music history, ethnomusicology.
            ...
            price_per_use=1.5,
            tags=["music", "music", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute music analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MusicInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Music Theory Expert with deep knowledge in music theory, composition, harmony, music history, ethnomusicology.

Your role is to:
1. Provide comprehensive analysis of music queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MusicInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        ...
        if not methodologies:
            methodologies = ["Standard music analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "music",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["music"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in music",
