"""
Philosophy Specialist - Expert in expert ai for philosophical analysis, ethics, and philosophical traditions
"""
...
class PhilosophyInput(BaseModel):
    """Input schema for philosophy queries."""
    query: str = Field(..., description="Philosophy related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within philosophy")
...
class PhilosophyOutput(BaseModel):
    """Output schema for philosophy analysis."""
    analysis: str = Field(..., description="Comprehensive philosophy analysis and insights")
...
class PhilosophyAgent(BaseAgent):
    """
    Specialized AI agent for philosophy.
    Expert AI for philosophical analysis, ethics, and philosophical traditions.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Philosophy Specialist",
            description="""Expert AI for philosophical analysis, ethics, and philosophical traditions.
            Specializes in ethics, metaphysics, epistemology, logic, philosophy of mind, political philosophy.
            ...
            price_per_use=1.5,
            tags=["philosophy", "philosophy", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive philosophy analysis",
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute philosophy analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PhilosophyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Philosophy Specialist with deep knowledge in ethics, metaphysics, epistemology, logic, philosophy of mind, political philosophy.

Your role is to:
1. Provide comprehensive analysis of philosophy queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PhilosophyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: PhilosophyInput
    ) -> PhilosophyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard philosophy analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "philosophy",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["philosophy"]
        }
