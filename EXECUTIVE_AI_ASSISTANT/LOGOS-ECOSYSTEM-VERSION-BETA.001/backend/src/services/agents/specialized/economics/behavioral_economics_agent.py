"""
Behavioral Economics Expert - Expert in expert ai for psychological factors in economic decision-making
"""
...
class BehavioralEconomicsInput(BaseModel):
    """Input schema for behavioral economics queries."""
    query: str = Field(..., description="Behavioral Economics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within behavioral economics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class BehavioralEconomicsOutput(BaseModel):
    """Output schema for behavioral economics analysis."""
    analysis: str = Field(..., description="Comprehensive behavioral economics analysis and insights")
...
class BehavioralEconomicsAgent(BaseAgent):
    """
    Specialized AI agent for behavioral economics.
    Expert AI for psychological factors in economic decision-making.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Behavioral Economics Expert",
            description="""Expert AI for psychological factors in economic decision-making.
            Specializes in cognitive biases, prospect theory, nudge theory, experimental economics.
            ...
            price_per_use=2.0,
            tags=["behavioral-economics", "behavioral economics", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive behavioral economics analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return BehavioralEconomicsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return BehavioralEconomicsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute behavioral economics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = BehavioralEconomicsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Behavioral Economics Expert with deep knowledge in cognitive biases, prospect theory, nudge theory, experimental economics.

Your role is to:
1. Provide comprehensive analysis of behavioral economics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: BehavioralEconomicsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: BehavioralEconomicsInput
    ) -> BehavioralEconomicsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard behavioral economics analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "behavioral economics",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["behavioral economics"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in behavioral economics",
