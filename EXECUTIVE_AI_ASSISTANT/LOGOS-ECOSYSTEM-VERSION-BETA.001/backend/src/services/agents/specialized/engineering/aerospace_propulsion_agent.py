"""
Aerospace Propulsion Expert - Expert in expert ai for rocket engines, jet propulsion, and spacecraft propulsion systems
"""
...
class AerospacePropulsionInput(BaseModel):
    """Input schema for aerospace propulsion queries."""
    query: str = Field(..., description="Aerospace Propulsion related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within aerospace propulsion")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class AerospacePropulsionOutput(BaseModel):
    """Output schema for aerospace propulsion analysis."""
    analysis: str = Field(..., description="Comprehensive aerospace propulsion analysis and insights")
...
class AerospacePropulsionAgent(BaseAgent):
    """
    Specialized AI agent for aerospace propulsion.
    Expert AI for rocket engines, jet propulsion, and spacecraft propulsion systems.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Aerospace Propulsion Expert",
            description="""Expert AI for rocket engines, jet propulsion, and spacecraft propulsion systems.
            Specializes in rocket engines, jet engines, electric propulsion, propellants, thrust calculations.
            ...
            price_per_use=2.5,
            tags=["aerospace-propulsion", "aerospace propulsion", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive aerospace propulsion analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return AerospacePropulsionInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return AerospacePropulsionOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute aerospace propulsion analysis using Claude AI."""
        try:
            # Validate input
            validated_input = AerospacePropulsionInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Aerospace Propulsion Expert with deep knowledge in rocket engines, jet engines, electric propulsion, propellants, thrust calculations.

Your role is to:
1. Provide comprehensive analysis of aerospace propulsion queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: AerospacePropulsionInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: AerospacePropulsionInput
    ) -> AerospacePropulsionOutput:
        ...
        if not methodologies:
            methodologies = ["Standard aerospace propulsion analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "aerospace propulsion",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["aerospace propulsion"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in aerospace propulsion",
