"""
Biomedical Engineering Specialist - Expert in expert ai for medical devices, biomaterials, and bioengineering applications
"""
...
class BiomedicalEngineeringInput(BaseModel):
    """Input schema for biomedical engineering queries."""
    query: str = Field(..., description="Biomedical Engineering related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within biomedical engineering")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class BiomedicalEngineeringOutput(BaseModel):
    """Output schema for biomedical engineering analysis."""
    analysis: str = Field(..., description="Comprehensive biomedical engineering analysis and insights")
...
class BiomedicalEngineeringAgent(BaseAgent):
    """
    Specialized AI agent for biomedical engineering.
    Expert AI for medical devices, biomaterials, and bioengineering applications.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Biomedical Engineering Specialist",
            description="""Expert AI for medical devices, biomaterials, and bioengineering applications.
            Specializes in medical imaging, prosthetics, biomaterials, tissue engineering, medical instrumentation.
            Provides detailed analysis, expert insights, and practical recommendations.""",
            category=AgentCategory.SOFTWARE_DEVELOPMENT,
            version="1.0.0",
            author="LOGOS AI Team",
            pricing_model=PricingModel.PER_USE,
            price_per_use=2.5,
            tags=["biomedical-engineering", "biomedical engineering", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive biomedical engineering analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return BiomedicalEngineeringInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return BiomedicalEngineeringOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute biomedical engineering analysis using Claude AI."""
        try:
            # Validate input
            validated_input = BiomedicalEngineeringInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Biomedical Engineering Specialist with deep knowledge in medical imaging, prosthetics, biomaterials, tissue engineering, medical instrumentation.

Your role is to:
1. Provide comprehensive analysis of biomedical engineering queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: BiomedicalEngineeringInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: BiomedicalEngineeringInput
    ) -> BiomedicalEngineeringOutput:
        ...
        if not methodologies:
            methodologies = ["Standard biomedical engineering analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "biomedical engineering",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["biomedical engineering"]
        }
