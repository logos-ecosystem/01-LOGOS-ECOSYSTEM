"""
Machine Learning Engineering Expert - Expert in expert ai for ml system design, model deployment, and mlops practices
"""
...
class MachineLearningEngineeringInput(BaseModel):
    """Input schema for machine learning engineering queries."""
    query: str = Field(..., description="Machine Learning Engineering related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within machine learning engineering")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MachineLearningEngineeringOutput(BaseModel):
    """Output schema for machine learning engineering analysis."""
    analysis: str = Field(..., description="Comprehensive machine learning engineering analysis and insights")
...
class MachineLearningEngineeringAgent(BaseAgent):
    """
    Specialized AI agent for machine learning engineering.
    Expert AI for ML system design, model deployment, and MLOps practices.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Machine Learning Engineering Expert",
            description="""Expert AI for ML system design, model deployment, and MLOps practices.
            Specializes in model deployment, ML pipelines, feature engineering, model optimization, MLOps.
            ...
            price_per_use=2.5,
            tags=["machine-learning-engineering", "machine learning engineering", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive machine learning engineering analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return MachineLearningEngineeringInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return MachineLearningEngineeringOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute machine learning engineering analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MachineLearningEngineeringInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Machine Learning Engineering Expert with deep knowledge in model deployment, ML pipelines, feature engineering, model optimization, MLOps.

Your role is to:
1. Provide comprehensive analysis of machine learning engineering queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MachineLearningEngineeringInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: MachineLearningEngineeringInput
    ) -> MachineLearningEngineeringOutput:
        ...
        if not methodologies:
            methodologies = ["Standard machine learning engineering analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "machine learning engineering",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["machine learning engineering"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in machine learning engineering",
