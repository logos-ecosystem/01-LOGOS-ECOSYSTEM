"""
Chemical Engineering Expert - Expert in expert ai for chemical processes, reactor design, and process optimization
"""
...
class ChemicalEngineeringInput(BaseModel):
    """Input schema for chemical engineering queries."""
    query: str = Field(..., description="Chemical Engineering related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within chemical engineering")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ChemicalEngineeringOutput(BaseModel):
    """Output schema for chemical engineering analysis."""
    analysis: str = Field(..., description="Comprehensive chemical engineering analysis and insights")
...
class ChemicalEngineeringAgent(BaseAgent):
    """
    Specialized AI agent for chemical engineering.
    Expert AI for chemical processes, reactor design, and process optimization.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Chemical Engineering Expert",
            description="""Expert AI for chemical processes, reactor design, and process optimization.
            Specializes in process design, reaction engineering, separation processes, thermodynamics, transport phenomena.
            ...
            price_per_use=2.0,
            tags=["chemical-engineering", "chemical engineering", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive chemical engineering analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ChemicalEngineeringInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ChemicalEngineeringOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute chemical engineering analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ChemicalEngineeringInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Chemical Engineering Expert with deep knowledge in process design, reaction engineering, separation processes, thermodynamics, transport phenomena.

Your role is to:
1. Provide comprehensive analysis of chemical engineering queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ChemicalEngineeringInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ChemicalEngineeringInput
    ) -> ChemicalEngineeringOutput:
        ...
        if not methodologies:
            methodologies = ["Standard chemical engineering analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "chemical engineering",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["chemical engineering"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in chemical engineering",
