"""
Aerospace Engineering Specialist - Expert in expert ai for aircraft design, spacecraft engineering, and aerodynamics
"""
...
class AerospaceEngineeringInput(BaseModel):
    """Input schema for aerospace engineering queries."""
    query: str = Field(..., description="Aerospace Engineering related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within aerospace engineering")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class AerospaceEngineeringOutput(BaseModel):
    """Output schema for aerospace engineering analysis."""
    analysis: str = Field(..., description="Comprehensive aerospace engineering analysis and insights")
...
class AerospaceEngineeringAgent(BaseAgent):
    """
    Specialized AI agent for aerospace engineering.
    Expert AI for aircraft design, spacecraft engineering, and aerodynamics.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Aerospace Engineering Specialist",
            description="""Expert AI for aircraft design, spacecraft engineering, and aerodynamics.
            Specializes in aerodynamics, propulsion, structures, flight dynamics, spacecraft design.
            ...
            price_per_use=2.5,
            tags=["aerospace-engineering", "aerospace engineering", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive aerospace engineering analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return AerospaceEngineeringInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return AerospaceEngineeringOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute aerospace engineering analysis using Claude AI."""
        try:
            # Validate input
            validated_input = AerospaceEngineeringInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Aerospace Engineering Specialist with deep knowledge in aerodynamics, propulsion, structures, flight dynamics, spacecraft design.

Your role is to:
1. Provide comprehensive analysis of aerospace engineering queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: AerospaceEngineeringInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: AerospaceEngineeringInput
    ) -> AerospaceEngineeringOutput:
        ...
        if not methodologies:
            methodologies = ["Standard aerospace engineering analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "aerospace engineering",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["aerospace engineering"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in aerospace engineering",
