"""
Mechanical Engineering Expert - Expert in expert ai for mechanical design, thermodynamics, and materials mechanics
"""
...
class MechanicalEngineeringInput(BaseModel):
    """Input schema for mechanical engineering queries."""
    query: str = Field(..., description="Mechanical Engineering related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within mechanical engineering")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MechanicalEngineeringOutput(BaseModel):
    """Output schema for mechanical engineering analysis."""
    analysis: str = Field(..., description="Comprehensive mechanical engineering analysis and insights")
...
class MechanicalEngineeringAgent(BaseAgent):
    """
    Specialized AI agent for mechanical engineering.
    Expert AI for mechanical design, thermodynamics, and materials mechanics.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Mechanical Engineering Expert",
            description="""Expert AI for mechanical design, thermodynamics, and materials mechanics.
            Specializes in machine design, thermodynamics, fluid mechanics, materials science, manufacturing.
            ...
            price_per_use=2.0,
            tags=["mechanical-engineering", "mechanical engineering", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive mechanical engineering analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return MechanicalEngineeringInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return MechanicalEngineeringOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute mechanical engineering analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MechanicalEngineeringInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Mechanical Engineering Expert with deep knowledge in machine design, thermodynamics, fluid mechanics, materials science, manufacturing.

Your role is to:
1. Provide comprehensive analysis of mechanical engineering queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MechanicalEngineeringInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: MechanicalEngineeringInput
    ) -> MechanicalEngineeringOutput:
        ...
        if not methodologies:
            methodologies = ["Standard mechanical engineering analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "mechanical engineering",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["mechanical engineering"]
        }
