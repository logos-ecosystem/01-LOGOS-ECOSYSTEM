"""
Electrical Engineering Specialist - Expert in expert ai for electrical systems, circuits, and power engineering
"""
...
class ElectricalEngineeringInput(BaseModel):
    """Input schema for electrical engineering queries."""
    query: str = Field(..., description="Electrical Engineering related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within electrical engineering")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ElectricalEngineeringOutput(BaseModel):
    """Output schema for electrical engineering analysis."""
    analysis: str = Field(..., description="Comprehensive electrical engineering analysis and insights")
...
class ElectricalEngineeringAgent(BaseAgent):
    """
    Specialized AI agent for electrical engineering.
    Expert AI for electrical systems, circuits, and power engineering.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Electrical Engineering Specialist",
            description="""Expert AI for electrical systems, circuits, and power engineering.
            Specializes in circuit design, power systems, electronics, control systems, electromagnetics.
            ...
            price_per_use=2.0,
            tags=["electrical-engineering", "electrical engineering", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive electrical engineering analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ElectricalEngineeringInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ElectricalEngineeringOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute electrical engineering analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ElectricalEngineeringInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Electrical Engineering Specialist with deep knowledge in circuit design, power systems, electronics, control systems, electromagnetics.

Your role is to:
1. Provide comprehensive analysis of electrical engineering queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ElectricalEngineeringInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ElectricalEngineeringInput
    ) -> ElectricalEngineeringOutput:
        ...
        if not methodologies:
            methodologies = ["Standard electrical engineering analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "electrical engineering",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["electrical engineering"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in electrical engineering",
