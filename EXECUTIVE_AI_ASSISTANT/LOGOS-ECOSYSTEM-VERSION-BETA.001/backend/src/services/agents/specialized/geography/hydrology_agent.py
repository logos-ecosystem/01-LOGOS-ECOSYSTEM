"""
Hydrology Specialist - Expert in expert ai for water systems, hydrological cycles, and water resource management
"""
...
class HydrologyInput(BaseModel):
    """Input schema for hydrology queries."""
    query: str = Field(..., description="Hydrology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within hydrology")
...
class HydrologyOutput(BaseModel):
    """Output schema for hydrology analysis."""
    analysis: str = Field(..., description="Comprehensive hydrology analysis and insights")
...
class HydrologyAgent(BaseAgent):
    """
    Specialized AI agent for hydrology.
    Expert AI for water systems, hydrological cycles, and water resource management.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Hydrology Specialist",
            description="""Expert AI for water systems, hydrological cycles, and water resource management.
            Specializes in surface water, groundwater, water cycle, watershed management, flood analysis.
            ...
            price_per_use=2.0,
            tags=["hydrology", "hydrology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive hydrology analysis",
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute hydrology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = HydrologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Hydrology Specialist with deep knowledge in surface water, groundwater, water cycle, watershed management, flood analysis.

Your role is to:
1. Provide comprehensive analysis of hydrology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: HydrologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: HydrologyInput
    ) -> HydrologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard hydrology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "hydrology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["hydrology"]
        }
