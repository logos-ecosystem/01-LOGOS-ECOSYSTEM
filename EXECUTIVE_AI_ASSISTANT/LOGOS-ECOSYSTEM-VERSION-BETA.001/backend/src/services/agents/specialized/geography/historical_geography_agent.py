"""
Historical Geography Expert - Expert in expert ai for geographic change over time, historical landscapes, and cultural evolution
"""
...
class HistoricalGeographyInput(BaseModel):
    """Input schema for historical geography queries."""
    query: str = Field(..., description="Historical Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within historical geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class HistoricalGeographyOutput(BaseModel):
    """Output schema for historical geography analysis."""
    analysis: str = Field(..., description="Comprehensive historical geography analysis and insights")
...
class HistoricalGeographyAgent(BaseAgent):
    """
    Specialized AI agent for historical geography.
    Expert AI for geographic change over time, historical landscapes, and cultural evolution.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Historical Geography Expert",
            description="""Expert AI for geographic change over time, historical landscapes, and cultural evolution.
            Specializes in landscape evolution, historical maps, cultural landscapes, geographic history.
            ...
            price_per_use=1.5,
            tags=["historical-geography", "historical geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive historical geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return HistoricalGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return HistoricalGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute historical geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = HistoricalGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Historical Geography Expert with deep knowledge in landscape evolution, historical maps, cultural landscapes, geographic history.

Your role is to:
1. Provide comprehensive analysis of historical geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: HistoricalGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: HistoricalGeographyInput
    ) -> HistoricalGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard historical geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "historical geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["historical geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in historical geography",
