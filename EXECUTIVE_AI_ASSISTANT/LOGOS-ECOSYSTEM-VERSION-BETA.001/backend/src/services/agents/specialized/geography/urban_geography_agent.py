"""
Urban Geography Specialist - Expert in expert ai for urban systems, city planning, and metropolitan dynamics
"""
...
class UrbanGeographyInput(BaseModel):
    """Input schema for urban geography queries."""
    query: str = Field(..., description="Urban Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within urban geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class UrbanGeographyOutput(BaseModel):
    """Output schema for urban geography analysis."""
    analysis: str = Field(..., description="Comprehensive urban geography analysis and insights")
...
class UrbanGeographyAgent(BaseAgent):
    """
    Specialized AI agent for urban geography.
    Expert AI for urban systems, city planning, and metropolitan dynamics.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Urban Geography Specialist",
            description="""Expert AI for urban systems, city planning, and metropolitan dynamics.
            Specializes in urbanization, city planning, urban economics, gentrification, smart cities.
            ...
            price_per_use=1.5,
            tags=["urban-geography", "urban geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive urban geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return UrbanGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return UrbanGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute urban geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = UrbanGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Urban Geography Specialist with deep knowledge in urbanization, city planning, urban economics, gentrification, smart cities.

Your role is to:
1. Provide comprehensive analysis of urban geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: UrbanGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: UrbanGeographyInput
    ) -> UrbanGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard urban geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "urban geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["urban geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in urban geography",
