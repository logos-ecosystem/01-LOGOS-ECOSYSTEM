"""
Population Geography Expert - Expert in expert ai for demographic patterns, migration, and population distribution
"""
...
class PopulationGeographyInput(BaseModel):
    """Input schema for population geography queries."""
    query: str = Field(..., description="Population Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within population geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class PopulationGeographyOutput(BaseModel):
    """Output schema for population geography analysis."""
    analysis: str = Field(..., description="Comprehensive population geography analysis and insights")
...
class PopulationGeographyAgent(BaseAgent):
    """
    Specialized AI agent for population geography.
    Expert AI for demographic patterns, migration, and population distribution.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Population Geography Expert",
            description="""Expert AI for demographic patterns, migration, and population distribution.
            Specializes in demographics, migration patterns, urbanization, population dynamics, census analysis.
            ...
            price_per_use=1.5,
            tags=["population-geography", "population geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive population geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return PopulationGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return PopulationGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute population geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PopulationGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Population Geography Expert with deep knowledge in demographics, migration patterns, urbanization, population dynamics, census analysis.

Your role is to:
1. Provide comprehensive analysis of population geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PopulationGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: PopulationGeographyInput
    ) -> PopulationGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard population geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "population geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["population geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in population geography",
