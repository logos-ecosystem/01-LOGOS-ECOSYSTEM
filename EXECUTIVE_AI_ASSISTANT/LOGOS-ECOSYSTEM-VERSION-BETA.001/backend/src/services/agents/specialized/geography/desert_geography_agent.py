"""
Desert Geography Specialist - Expert in expert ai for arid environments, desert ecosystems, and desertification
"""
...
class DesertGeographyInput(BaseModel):
    """Input schema for desert geography queries."""
    query: str = Field(..., description="Desert Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within desert geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class DesertGeographyOutput(BaseModel):
    """Output schema for desert geography analysis."""
    analysis: str = Field(..., description="Comprehensive desert geography analysis and insights")
...
class DesertGeographyAgent(BaseAgent):
    """
    Specialized AI agent for desert geography.
    Expert AI for arid environments, desert ecosystems, and desertification.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Desert Geography Specialist",
            description="""Expert AI for arid environments, desert ecosystems, and desertification.
            Specializes in desert ecosystems, arid geomorphology, desertification, desert resources.
            ...
            price_per_use=1.5,
            tags=["desert-geography", "desert geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive desert geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return DesertGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return DesertGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute desert geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = DesertGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Desert Geography Specialist with deep knowledge in desert ecosystems, arid geomorphology, desertification, desert resources.

Your role is to:
1. Provide comprehensive analysis of desert geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: DesertGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: DesertGeographyInput
    ) -> DesertGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard desert geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "desert geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["desert geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in desert geography",
