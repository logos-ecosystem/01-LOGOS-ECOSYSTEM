"""
Economic Geography Expert - Expert in expert ai for spatial economics, regional development, and economic landscapes
"""
...
class EconomicGeographyInput(BaseModel):
    """Input schema for economic geography queries."""
    query: str = Field(..., description="Economic Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within economic geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class EconomicGeographyOutput(BaseModel):
    """Output schema for economic geography analysis."""
    analysis: str = Field(..., description="Comprehensive economic geography analysis and insights")
...
class EconomicGeographyAgent(BaseAgent):
    """
    Specialized AI agent for economic geography.
    Expert AI for spatial economics, regional development, and economic landscapes.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Economic Geography Expert",
            description="""Expert AI for spatial economics, regional development, and economic landscapes.
            Specializes in spatial economics, industrial location, regional development, globalization.
            ...
            price_per_use=1.5,
            tags=["economic-geography", "economic geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive economic geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return EconomicGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return EconomicGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute economic geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = EconomicGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Economic Geography Expert with deep knowledge in spatial economics, industrial location, regional development, globalization.

Your role is to:
1. Provide comprehensive analysis of economic geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: EconomicGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: EconomicGeographyInput
    ) -> EconomicGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard economic geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "economic geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["economic geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in economic geography",
