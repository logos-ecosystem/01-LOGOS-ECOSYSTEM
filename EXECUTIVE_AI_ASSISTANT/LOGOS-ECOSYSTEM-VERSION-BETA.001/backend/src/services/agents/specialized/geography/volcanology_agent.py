"""
Volcanology Expert - Expert in expert ai for volcanic processes, eruption prediction, and volcanic hazards
"""
...
class VolcanologyInput(BaseModel):
    """Input schema for volcanology queries."""
    query: str = Field(..., description="Volcanology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within volcanology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class VolcanologyOutput(BaseModel):
    """Output schema for volcanology analysis."""
    analysis: str = Field(..., description="Comprehensive volcanology analysis and insights")
...
class VolcanologyAgent(BaseAgent):
    """
    Specialized AI agent for volcanology.
    Expert AI for volcanic processes, eruption prediction, and volcanic hazards.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Volcanology Expert",
            description="""Expert AI for volcanic processes, eruption prediction, and volcanic hazards.
            Specializes in volcanic processes, eruption dynamics, hazard assessment, magma chemistry, monitoring.
            ...
            price_per_use=2.0,
            tags=["volcanology", "volcanology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive volcanology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return VolcanologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return VolcanologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute volcanology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = VolcanologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Volcanology Expert with deep knowledge in volcanic processes, eruption dynamics, hazard assessment, magma chemistry, monitoring.

Your role is to:
1. Provide comprehensive analysis of volcanology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: VolcanologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: VolcanologyInput
    ) -> VolcanologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard volcanology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "volcanology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["volcanology"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in volcanology",
