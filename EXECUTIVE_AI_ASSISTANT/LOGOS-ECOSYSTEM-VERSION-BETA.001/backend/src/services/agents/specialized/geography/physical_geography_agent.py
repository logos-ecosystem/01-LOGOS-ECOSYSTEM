"""
Physical Geography Specialist - Expert in expert ai for earth's physical systems, landforms, and natural processes
"""
...
class PhysicalGeographyInput(BaseModel):
    """Input schema for physical geography queries."""
    query: str = Field(..., description="Physical Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within physical geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class PhysicalGeographyOutput(BaseModel):
    """Output schema for physical geography analysis."""
    analysis: str = Field(..., description="Comprehensive physical geography analysis and insights")
...
class PhysicalGeographyAgent(BaseAgent):
    """
    Specialized AI agent for physical geography.
    Expert AI for Earth's physical systems, landforms, and natural processes.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Physical Geography Specialist",
            description="""Expert AI for Earth's physical systems, landforms, and natural processes.
            Specializes in geomorphology, climatology, biogeography, hydrology, natural hazards.
            ...
            price_per_use=1.5,
            tags=["physical-geography", "physical geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive physical geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return PhysicalGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return PhysicalGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute physical geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PhysicalGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Physical Geography Specialist with deep knowledge in geomorphology, climatology, biogeography, hydrology, natural hazards.

Your role is to:
1. Provide comprehensive analysis of physical geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PhysicalGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: PhysicalGeographyInput
    ) -> PhysicalGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard physical geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "physical geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["physical geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in physical geography",
