"""
Social Geography Expert - Expert in expert ai for social spatial patterns, inequality, and community geography
"""
...
class SocialGeographyInput(BaseModel):
    """Input schema for social geography queries."""
    query: str = Field(..., description="Social Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within social geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class SocialGeographyOutput(BaseModel):
    """Output schema for social geography analysis."""
    analysis: str = Field(..., description="Comprehensive social geography analysis and insights")
...
class SocialGeographyAgent(BaseAgent):
    """
    Specialized AI agent for social geography.
    Expert AI for social spatial patterns, inequality, and community geography.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Social Geography Expert",
            description="""Expert AI for social spatial patterns, inequality, and community geography.
            Specializes in social inequality, segregation, community studies, social movements, identity.
            ...
            price_per_use=1.5,
            tags=["social-geography", "social geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive social geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return SocialGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return SocialGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute social geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = SocialGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Social Geography Expert with deep knowledge in social inequality, segregation, community studies, social movements, identity.

Your role is to:
1. Provide comprehensive analysis of social geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: SocialGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: SocialGeographyInput
    ) -> SocialGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard social geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "social geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["social geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in social geography",
