"""
Political Geography Specialist - Expert in expert ai for geopolitics, territorial boundaries, and political landscapes
"""
...
class PoliticalGeographyInput(BaseModel):
    """Input schema for political geography queries."""
    query: str = Field(..., description="Political Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within political geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class PoliticalGeographyOutput(BaseModel):
    """Output schema for political geography analysis."""
    analysis: str = Field(..., description="Comprehensive political geography analysis and insights")
...
class PoliticalGeographyAgent(BaseAgent):
    """
    Specialized AI agent for political geography.
    Expert AI for geopolitics, territorial boundaries, and political landscapes.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Political Geography Specialist",
            description="""Expert AI for geopolitics, territorial boundaries, and political landscapes.
            Specializes in geopolitics, borders, territorial disputes, electoral geography, nationalism.
            ...
            price_per_use=1.5,
            tags=["political-geography", "political geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive political geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return PoliticalGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return PoliticalGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute political geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PoliticalGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Political Geography Specialist with deep knowledge in geopolitics, borders, territorial disputes, electoral geography, nationalism.

Your role is to:
1. Provide comprehensive analysis of political geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PoliticalGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: PoliticalGeographyInput
    ) -> PoliticalGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard political geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "political geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["political geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in political geography",
