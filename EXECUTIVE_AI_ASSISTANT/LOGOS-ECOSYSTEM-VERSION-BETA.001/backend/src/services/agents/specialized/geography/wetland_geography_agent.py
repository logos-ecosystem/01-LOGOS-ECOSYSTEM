"""
Wetland Geography Specialist - Expert in expert ai for wetland ecosystems, hydrology, and conservation
"""
...
class WetlandGeographyInput(BaseModel):
    """Input schema for wetland geography queries."""
    query: str = Field(..., description="Wetland Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within wetland geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class WetlandGeographyOutput(BaseModel):
    """Output schema for wetland geography analysis."""
    analysis: str = Field(..., description="Comprehensive wetland geography analysis and insights")
...
class WetlandGeographyAgent(BaseAgent):
    """
    Specialized AI agent for wetland geography.
    Expert AI for wetland ecosystems, hydrology, and conservation.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Wetland Geography Specialist",
            description="""Expert AI for wetland ecosystems, hydrology, and conservation.
            Specializes in wetland ecology, hydrology, conservation, ecosystem services, restoration.
            ...
            price_per_use=1.5,
            tags=["wetland-geography", "wetland geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive wetland geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return WetlandGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return WetlandGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute wetland geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = WetlandGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Wetland Geography Specialist with deep knowledge in wetland ecology, hydrology, conservation, ecosystem services, restoration.

Your role is to:
1. Provide comprehensive analysis of wetland geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: WetlandGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: WetlandGeographyInput
    ) -> WetlandGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard wetland geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "wetland geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["wetland geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in wetland geography",
