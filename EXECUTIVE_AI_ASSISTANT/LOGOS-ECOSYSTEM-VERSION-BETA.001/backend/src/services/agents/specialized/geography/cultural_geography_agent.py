"""
Cultural Geography Specialist - Expert in expert ai for cultural landscapes, human-environment interactions, and cultural patterns
"""
...
class CulturalGeographyInput(BaseModel):
    """Input schema for cultural geography queries."""
    query: str = Field(..., description="Cultural Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within cultural geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class CulturalGeographyOutput(BaseModel):
    """Output schema for cultural geography analysis."""
    analysis: str = Field(..., description="Comprehensive cultural geography analysis and insights")
...
class CulturalGeographyAgent(BaseAgent):
    """
    Specialized AI agent for cultural geography.
    Expert AI for cultural landscapes, human-environment interactions, and cultural patterns.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Cultural Geography Specialist",
            description="""Expert AI for cultural landscapes, human-environment interactions, and cultural patterns.
            Specializes in cultural landscapes, language geography, religion geography, cultural diffusion.
            ...
            price_per_use=1.5,
            tags=["cultural-geography", "cultural geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive cultural geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return CulturalGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return CulturalGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute cultural geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = CulturalGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Cultural Geography Specialist with deep knowledge in cultural landscapes, language geography, religion geography, cultural diffusion.

Your role is to:
1. Provide comprehensive analysis of cultural geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: CulturalGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: CulturalGeographyInput
    ) -> CulturalGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard cultural geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "cultural geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["cultural geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in cultural geography",
