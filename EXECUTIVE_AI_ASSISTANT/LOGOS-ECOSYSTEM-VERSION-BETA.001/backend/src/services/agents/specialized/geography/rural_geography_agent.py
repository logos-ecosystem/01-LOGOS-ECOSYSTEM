"""
Rural Geography Specialist - Expert in expert ai for rural landscapes, agricultural geography, and rural development
"""
...
class RuralGeographyInput(BaseModel):
    """Input schema for rural geography queries."""
    query: str = Field(..., description="Rural Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within rural geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class RuralGeographyOutput(BaseModel):
    """Output schema for rural geography analysis."""
    analysis: str = Field(..., description="Comprehensive rural geography analysis and insights")
...
class RuralGeographyAgent(BaseAgent):
    """
    Specialized AI agent for rural geography.
    Expert AI for rural landscapes, agricultural geography, and rural development.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Rural Geography Specialist",
            description="""Expert AI for rural landscapes, agricultural geography, and rural development.
            Specializes in rural development, agricultural systems, rural communities, land use change.
            ...
            price_per_use=1.5,
            tags=["rural-geography", "rural geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive rural geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return RuralGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return RuralGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute rural geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = RuralGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Rural Geography Specialist with deep knowledge in rural development, agricultural systems, rural communities, land use change.

Your role is to:
1. Provide comprehensive analysis of rural geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: RuralGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: RuralGeographyInput
    ) -> RuralGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard rural geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "rural geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["rural geography"]
        }
