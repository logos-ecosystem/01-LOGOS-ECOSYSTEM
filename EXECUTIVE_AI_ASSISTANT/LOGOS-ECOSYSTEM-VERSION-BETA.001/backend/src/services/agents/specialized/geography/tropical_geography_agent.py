"""
Tropical Geography Expert - Expert in expert ai for tropical environments, rainforests, and tropical development
"""
...
class TropicalGeographyInput(BaseModel):
    """Input schema for tropical geography queries."""
    query: str = Field(..., description="Tropical Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within tropical geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class TropicalGeographyOutput(BaseModel):
    """Output schema for tropical geography analysis."""
    analysis: str = Field(..., description="Comprehensive tropical geography analysis and insights")
...
class TropicalGeographyAgent(BaseAgent):
    """
    Specialized AI agent for tropical geography.
    Expert AI for tropical environments, rainforests, and tropical development.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Tropical Geography Expert",
            description="""Expert AI for tropical environments, rainforests, and tropical development.
            Specializes in tropical ecosystems, rainforests, tropical agriculture, monsoons, biodiversity.
            ...
            price_per_use=1.5,
            tags=["tropical-geography", "tropical geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive tropical geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return TropicalGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return TropicalGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute tropical geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = TropicalGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Tropical Geography Expert with deep knowledge in tropical ecosystems, rainforests, tropical agriculture, monsoons, biodiversity.

Your role is to:
1. Provide comprehensive analysis of tropical geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: TropicalGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: TropicalGeographyInput
    ) -> TropicalGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard tropical geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "tropical geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["tropical geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in tropical geography",
