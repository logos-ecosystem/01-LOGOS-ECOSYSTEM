"""
Pedology Expert - Expert in expert ai for soil science, soil formation, and soil classification
"""
...
class PedologyInput(BaseModel):
    """Input schema for pedology queries."""
    query: str = Field(..., description="Pedology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within pedology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class PedologyOutput(BaseModel):
    """Output schema for pedology analysis."""
    analysis: str = Field(..., description="Comprehensive pedology analysis and insights")
...
class PedologyAgent(BaseAgent):
    """
    Specialized AI agent for pedology.
    Expert AI for soil science, soil formation, and soil classification.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Pedology Expert",
            description="""Expert AI for soil science, soil formation, and soil classification.
            Specializes in soil formation, soil classification, soil chemistry, soil ecology, land use.
            ...
            price_per_use=1.5,
            tags=["pedology", "pedology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive pedology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return PedologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return PedologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute pedology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PedologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Pedology Expert with deep knowledge in soil formation, soil classification, soil chemistry, soil ecology, land use.

Your role is to:
1. Provide comprehensive analysis of pedology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PedologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        ...
        if not methodologies:
            methodologies = ["Standard pedology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "pedology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["pedology"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in pedology",
