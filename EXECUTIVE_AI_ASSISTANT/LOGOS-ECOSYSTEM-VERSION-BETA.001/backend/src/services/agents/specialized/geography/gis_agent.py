"""
GIS Specialist - Expert in expert ai for geographic information systems, spatial analysis, and mapping
"""
...
class GisInput(BaseModel):
    """Input schema for gis queries."""
    query: str = Field(..., description="Gis related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within gis")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class GisOutput(BaseModel):
    """Output schema for gis analysis."""
    analysis: str = Field(..., description="Comprehensive gis analysis and insights")
...
class GisAgent(BaseAgent):
    """
    Specialized AI agent for gis.
    Expert AI for Geographic Information Systems, spatial analysis, and mapping.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="GIS Specialist",
            description="""Expert AI for Geographic Information Systems, spatial analysis, and mapping.
            Specializes in spatial analysis, remote sensing, geodatabases, cartography, spatial modeling.
            ...
            price_per_use=2.0,
            tags=["gis", "gis", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive gis analysis",
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute gis analysis using Claude AI."""
        try:
            # Validate input
            validated_input = GisInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert GIS Specialist with deep knowledge in spatial analysis, remote sensing, geodatabases, cartography, spatial modeling.

Your role is to:
1. Provide comprehensive analysis of gis queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: GisInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        ...
        if not methodologies:
            methodologies = ["Standard gis analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "gis",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["gis"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in gis",
