"""
Mountain Geography Specialist - Expert in expert ai for mountain environments, alpine ecosystems, and mountain hazards
"""
...
class MountainGeographyInput(BaseModel):
    """Input schema for mountain geography queries."""
    query: str = Field(..., description="Mountain Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within mountain geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MountainGeographyOutput(BaseModel):
    """Output schema for mountain geography analysis."""
    analysis: str = Field(..., description="Comprehensive mountain geography analysis and insights")
...
class MountainGeographyAgent(BaseAgent):
    """
    Specialized AI agent for mountain geography.
    Expert AI for mountain environments, alpine ecosystems, and mountain hazards.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Mountain Geography Specialist",
            description="""Expert AI for mountain environments, alpine ecosystems, and mountain hazards.
            Specializes in mountain ecosystems, glaciology, avalanches, mountain climate, alpine geology.
            ...
            price_per_use=1.5,
            tags=["mountain-geography", "mountain geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive mountain geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return MountainGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return MountainGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute mountain geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MountainGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Mountain Geography Specialist with deep knowledge in mountain ecosystems, glaciology, avalanches, mountain climate, alpine geology.

Your role is to:
1. Provide comprehensive analysis of mountain geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MountainGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: MountainGeographyInput
    ) -> MountainGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard mountain geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "mountain geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["mountain geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in mountain geography",
