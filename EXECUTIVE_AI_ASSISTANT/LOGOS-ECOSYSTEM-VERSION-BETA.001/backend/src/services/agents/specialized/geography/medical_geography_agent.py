"""
Medical Geography Expert - Expert in expert ai for health geography, disease mapping, and spatial epidemiology
"""
...
class MedicalGeographyInput(BaseModel):
    """Input schema for medical geography queries."""
    query: str = Field(..., description="Medical Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within medical geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MedicalGeographyOutput(BaseModel):
    """Output schema for medical geography analysis."""
    analysis: str = Field(..., description="Comprehensive medical geography analysis and insights")
...
class MedicalGeographyAgent(BaseAgent):
    """
    Specialized AI agent for medical geography.
    Expert AI for health geography, disease mapping, and spatial epidemiology.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Medical Geography Expert",
            description="""Expert AI for health geography, disease mapping, and spatial epidemiology.
            Specializes in disease mapping, health disparities, spatial epidemiology, healthcare access.
            ...
            price_per_use=2.0,
            tags=["medical-geography", "medical geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive medical geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return MedicalGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return MedicalGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute medical geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MedicalGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Medical Geography Expert with deep knowledge in disease mapping, health disparities, spatial epidemiology, healthcare access.

Your role is to:
1. Provide comprehensive analysis of medical geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MedicalGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: MedicalGeographyInput
    ) -> MedicalGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard medical geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "medical geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["medical geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in medical geography",
