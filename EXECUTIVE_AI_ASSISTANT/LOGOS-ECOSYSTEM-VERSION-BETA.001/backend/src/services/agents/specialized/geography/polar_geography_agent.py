"""
Polar Geography Expert - Expert in expert ai for arctic and antarctic environments, ice dynamics, and polar ecosystems
"""
...
class PolarGeographyInput(BaseModel):
    """Input schema for polar geography queries."""
    query: str = Field(..., description="Polar Geography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within polar geography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class PolarGeographyOutput(BaseModel):
    """Output schema for polar geography analysis."""
    analysis: str = Field(..., description="Comprehensive polar geography analysis and insights")
...
class PolarGeographyAgent(BaseAgent):
    """
    Specialized AI agent for polar geography.
    Expert AI for Arctic and Antarctic environments, ice dynamics, and polar ecosystems.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Polar Geography Expert",
            description="""Expert AI for Arctic and Antarctic environments, ice dynamics, and polar ecosystems.
            Specializes in polar climate, ice sheets, permafrost, polar ecosystems, climate change impacts.
            ...
            price_per_use=2.0,
            tags=["polar-geography", "polar geography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive polar geography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return PolarGeographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return PolarGeographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute polar geography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = PolarGeographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Polar Geography Expert with deep knowledge in polar climate, ice sheets, permafrost, polar ecosystems, climate change impacts.

Your role is to:
1. Provide comprehensive analysis of polar geography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: PolarGeographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: PolarGeographyInput
    ) -> PolarGeographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard polar geography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "polar geography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["polar geography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in polar geography",
