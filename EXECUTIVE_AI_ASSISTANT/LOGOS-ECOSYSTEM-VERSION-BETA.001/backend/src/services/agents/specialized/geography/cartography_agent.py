"""
Cartography Expert - Expert in expert ai for map design, geographic visualization, and spatial representation
"""
...
class CartographyInput(BaseModel):
    """Input schema for cartography queries."""
    query: str = Field(..., description="Cartography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within cartography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class CartographyOutput(BaseModel):
    """Output schema for cartography analysis."""
    analysis: str = Field(..., description="Comprehensive cartography analysis and insights")
...
class CartographyAgent(BaseAgent):
    """
    Specialized AI agent for cartography.
    Expert AI for map design, geographic visualization, and spatial representation.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Cartography Expert",
            description="""Expert AI for map design, geographic visualization, and spatial representation.
            Specializes in map projections, thematic mapping, digital cartography, map design principles.
            ...
            price_per_use=1.5,
            tags=["cartography", "cartography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive cartography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return CartographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return CartographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute cartography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = CartographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Cartography Expert with deep knowledge in map projections, thematic mapping, digital cartography, map design principles.

Your role is to:
1. Provide comprehensive analysis of cartography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: CartographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: CartographyInput
    ) -> CartographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard cartography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "cartography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["cartography"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in cartography",
