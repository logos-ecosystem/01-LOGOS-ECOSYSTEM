"""
Organic Chemistry Expert - Expert in expert ai for organic synthesis, reaction mechanisms, and molecular structure
"""
...
class OrganicChemistryInput(BaseModel):
    """Input schema for organic chemistry queries."""
    query: str = Field(..., description="Organic Chemistry related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within organic chemistry")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class OrganicChemistryOutput(BaseModel):
    """Output schema for organic chemistry analysis."""
    analysis: str = Field(..., description="Comprehensive organic chemistry analysis and insights")
...
class OrganicChemistryAgent(BaseAgent):
    """
    Specialized AI agent for organic chemistry.
    Expert AI for organic synthesis, reaction mechanisms, and molecular structure.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Organic Chemistry Expert",
            description="""Expert AI for organic synthesis, reaction mechanisms, and molecular structure.
            Specializes in synthesis, reaction mechanisms, spectroscopy, medicinal chemistry, polymers.
            ...
            price_per_use=2.0,
            tags=["organic-chemistry", "organic chemistry", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive organic chemistry analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return OrganicChemistryInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return OrganicChemistryOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute organic chemistry analysis using Claude AI."""
        try:
            # Validate input
            validated_input = OrganicChemistryInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Organic Chemistry Expert with deep knowledge in synthesis, reaction mechanisms, spectroscopy, medicinal chemistry, polymers.

Your role is to:
1. Provide comprehensive analysis of organic chemistry queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: OrganicChemistryInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: OrganicChemistryInput
    ) -> OrganicChemistryOutput:
        ...
        if not methodologies:
            methodologies = ["Standard organic chemistry analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "organic chemistry",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["organic chemistry"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in organic chemistry",
