"""
Nuclear Physics Specialist - Expert in expert ai for nuclear structure, radioactivity, and nuclear reactions
"""
...
class NuclearPhysicsInput(BaseModel):
    """Input schema for nuclear physics queries."""
    query: str = Field(..., description="Nuclear Physics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within nuclear physics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class NuclearPhysicsOutput(BaseModel):
    """Output schema for nuclear physics analysis."""
    analysis: str = Field(..., description="Comprehensive nuclear physics analysis and insights")
...
class NuclearPhysicsAgent(BaseAgent):
    """
    Specialized AI agent for nuclear physics.
    Expert AI for nuclear structure, radioactivity, and nuclear reactions.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Nuclear Physics Specialist",
            description="""Expert AI for nuclear structure, radioactivity, and nuclear reactions.
            Specializes in nuclear models, decay processes, fission/fusion, nuclear astrophysics, particle interactions.
            ...
            price_per_use=2.0,
            tags=["nuclear-physics", "nuclear physics", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive nuclear physics analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return NuclearPhysicsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return NuclearPhysicsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute nuclear physics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = NuclearPhysicsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Nuclear Physics Specialist with deep knowledge in nuclear models, decay processes, fission/fusion, nuclear astrophysics, particle interactions.

Your role is to:
1. Provide comprehensive analysis of nuclear physics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: NuclearPhysicsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: NuclearPhysicsInput
    ) -> NuclearPhysicsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard nuclear physics analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "nuclear physics",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["nuclear physics"]
        }
