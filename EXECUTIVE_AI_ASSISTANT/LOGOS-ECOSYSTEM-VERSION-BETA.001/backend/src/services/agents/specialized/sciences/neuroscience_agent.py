"""
Neuroscience Specialist - Expert in expert ai for brain function, neural mechanisms, and neuroscience research
"""
...
class NeuroscienceInput(BaseModel):
    """Input schema for neuroscience queries."""
    query: str = Field(..., description="Neuroscience related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within neuroscience")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class NeuroscienceOutput(BaseModel):
    """Output schema for neuroscience analysis."""
    analysis: str = Field(..., description="Comprehensive neuroscience analysis and insights")
...
class NeuroscienceAgent(BaseAgent):
    """
    Specialized AI agent for neuroscience.
    Expert AI for brain function, neural mechanisms, and neuroscience research.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Neuroscience Specialist",
            description="""Expert AI for brain function, neural mechanisms, and neuroscience research.
            Specializes in neural circuits, brain imaging, cognitive neuroscience, neuroplasticity.
            ...
            price_per_use=2.5,
            tags=["neuroscience", "neuroscience", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return NeuroscienceInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return NeuroscienceOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute neuroscience analysis using Claude AI."""
        try:
            # Validate input
            validated_input = NeuroscienceInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Neuroscience Specialist with deep knowledge in neural circuits, brain imaging, cognitive neuroscience, neuroplasticity.

Your role is to:
1. Provide comprehensive analysis of neuroscience queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: NeuroscienceInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: NeuroscienceInput
    ) -> NeuroscienceOutput:
        ...
        if not methodologies:
            methodologies = ["Standard neuroscience analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "neuroscience",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["neuroscience"]
        }
