"""
Particle Physics Expert - Expert in expert ai for fundamental particles, quantum field theory, and high-energy physics
"""
...
class ParticlePhysicsInput(BaseModel):
    """Input schema for particle physics queries."""
    query: str = Field(..., description="Particle Physics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within particle physics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ParticlePhysicsOutput(BaseModel):
    """Output schema for particle physics analysis."""
    analysis: str = Field(..., description="Comprehensive particle physics analysis and insights")
...
class ParticlePhysicsAgent(BaseAgent):
    """
    Specialized AI agent for particle physics.
    Expert AI for fundamental particles, quantum field theory, and high-energy physics.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Particle Physics Expert",
            description="""Expert AI for fundamental particles, quantum field theory, and high-energy physics.
            Specializes in Standard Model, particle accelerators, detector physics, symmetries, beyond Standard Model.
            ...
            price_per_use=2.0,
            tags=["particle-physics", "particle physics", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive particle physics analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ParticlePhysicsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ParticlePhysicsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute particle physics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ParticlePhysicsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Particle Physics Expert with deep knowledge in Standard Model, particle accelerators, detector physics, symmetries, beyond Standard Model.

Your role is to:
1. Provide comprehensive analysis of particle physics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ParticlePhysicsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ParticlePhysicsInput
    ) -> ParticlePhysicsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard particle physics analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "particle physics",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["particle physics"]
        }
