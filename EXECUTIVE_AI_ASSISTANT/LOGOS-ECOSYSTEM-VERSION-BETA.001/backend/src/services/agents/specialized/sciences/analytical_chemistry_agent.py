"""
Analytical Chemistry Expert - Expert in expert ai for analytical chemistry methods, instrumentation, and chemical analysis
"""
...
class AnalyticalChemistryInput(BaseModel):
    """Input schema for analytical chemistry queries."""
    query: str = Field(..., description="Analytical Chemistry related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within analytical chemistry")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class AnalyticalChemistryOutput(BaseModel):
    """Output schema for analytical chemistry analysis."""
    analysis: str = Field(..., description="Comprehensive analytical chemistry analysis and insights")
...
class AnalyticalChemistryAgent(BaseAgent):
    """
    Specialized AI agent for analytical chemistry.
    Expert AI for analytical chemistry methods, instrumentation, and chemical analysis.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Analytical Chemistry Expert",
            description="""Expert AI for analytical chemistry methods, instrumentation, and chemical analysis.
            Specializes in spectroscopy, chromatography, mass spectrometry, electrochemistry, chemical separations.
            ...
            price_per_use=2.0,
            tags=["analytical-chemistry", "analytical chemistry", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive analytical chemistry analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return AnalyticalChemistryInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return AnalyticalChemistryOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute analytical chemistry analysis using Claude AI."""
        try:
            # Validate input
            validated_input = AnalyticalChemistryInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Analytical Chemistry Expert with deep knowledge in spectroscopy, chromatography, mass spectrometry, electrochemistry, chemical separations.

Your role is to:
1. Provide comprehensive analysis of analytical chemistry queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: AnalyticalChemistryInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: AnalyticalChemistryInput
    ) -> AnalyticalChemistryOutput:
        ...
        if not methodologies:
            methodologies = ["Standard analytical chemistry analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "analytical chemistry",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["analytical chemistry"]
        }
