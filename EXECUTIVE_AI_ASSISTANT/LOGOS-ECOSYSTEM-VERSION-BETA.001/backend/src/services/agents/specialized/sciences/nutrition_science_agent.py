"""
Nutrition Science Expert - Expert in expert ai for nutritional science, dietary planning, and metabolic health
"""
...
class NutritionScienceInput(BaseModel):
    """Input schema for nutrition science queries."""
    query: str = Field(..., description="Nutrition Science related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within nutrition science")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class NutritionScienceOutput(BaseModel):
    """Output schema for nutrition science analysis."""
    analysis: str = Field(..., description="Comprehensive nutrition science analysis and insights")
...
class NutritionScienceAgent(BaseAgent):
    """
    Specialized AI agent for nutrition science.
    Expert AI for nutritional science, dietary planning, and metabolic health.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Nutrition Science Expert",
            description="""Expert AI for nutritional science, dietary planning, and metabolic health.
            Specializes in macronutrients, micronutrients, dietary guidelines, metabolic disorders, nutritional biochemistry.
            ...
            price_per_use=2.0,
            tags=["nutrition-science", "nutrition science", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive nutrition science analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return NutritionScienceInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return NutritionScienceOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute nutrition science analysis using Claude AI."""
        try:
            # Validate input
            validated_input = NutritionScienceInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Nutrition Science Expert with deep knowledge in macronutrients, micronutrients, dietary guidelines, metabolic disorders, nutritional biochemistry.

Your role is to:
1. Provide comprehensive analysis of nutrition science queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: NutritionScienceInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: NutritionScienceInput
    ) -> NutritionScienceOutput:
        ...
        if not methodologies:
            methodologies = ["Standard nutrition science analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "nutrition science",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["nutrition science"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in nutrition science",
