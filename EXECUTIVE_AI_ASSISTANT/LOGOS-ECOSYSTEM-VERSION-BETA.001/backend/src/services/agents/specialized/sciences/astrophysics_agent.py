"""
Astrophysics Specialist - Expert in expert ai for stellar physics, cosmology, and high-energy astrophysics
"""
...
class AstrophysicsInput(BaseModel):
    """Input schema for astrophysics queries."""
    query: str = Field(..., description="Astrophysics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within astrophysics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class AstrophysicsOutput(BaseModel):
    """Output schema for astrophysics analysis."""
    analysis: str = Field(..., description="Comprehensive astrophysics analysis and insights")
...
class AstrophysicsAgent(BaseAgent):
    """
    Specialized AI agent for astrophysics.
    Expert AI for stellar physics, cosmology, and high-energy astrophysics.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Astrophysics Specialist",
            description="""Expert AI for stellar physics, cosmology, and high-energy astrophysics.
            Specializes in stellar evolution, cosmology, black holes, galaxies, high-energy phenomena.
            ...
            price_per_use=2.0,
            tags=["astrophysics", "astrophysics", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive astrophysics analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return AstrophysicsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return AstrophysicsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute astrophysics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = AstrophysicsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Astrophysics Specialist with deep knowledge in stellar evolution, cosmology, black holes, galaxies, high-energy phenomena.

Your role is to:
1. Provide comprehensive analysis of astrophysics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: AstrophysicsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: AstrophysicsInput
    ) -> AstrophysicsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard astrophysics analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "astrophysics",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["astrophysics"]
        }
