"""
Molecular Biology Expert - Expert in expert ai for molecular mechanisms, gene expression, and cellular processes
"""
...
class MolecularBiologyInput(BaseModel):
    """Input schema for molecular biology queries."""
    query: str = Field(..., description="Molecular Biology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within molecular biology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MolecularBiologyOutput(BaseModel):
    """Output schema for molecular biology analysis."""
    analysis: str = Field(..., description="Comprehensive molecular biology analysis and insights")
...
class MolecularBiologyAgent(BaseAgent):
    """
    Specialized AI agent for molecular biology.
    Expert AI for molecular mechanisms, gene expression, and cellular processes.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Molecular Biology Expert",
            description="""Expert AI for molecular mechanisms, gene expression, and cellular processes.
            Specializes in DNA/RNA, protein synthesis, gene regulation, molecular techniques, cell signaling.
            ...
            price_per_use=2.0,
            tags=["molecular-biology", "molecular biology", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return MolecularBiologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return MolecularBiologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute molecular biology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MolecularBiologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Molecular Biology Expert with deep knowledge in DNA/RNA, protein synthesis, gene regulation, molecular techniques, cell signaling.

Your role is to:
1. Provide comprehensive analysis of molecular biology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MolecularBiologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: MolecularBiologyInput
    ) -> MolecularBiologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard molecular biology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "molecular biology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["molecular biology"]
        }
