"""
Meteorology Specialist - Expert in expert ai for weather forecasting, atmospheric dynamics, and climate systems
"""
...
class MeteorologyInput(BaseModel):
    """Input schema for meteorology queries."""
    query: str = Field(..., description="Meteorology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within meteorology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MeteorologyOutput(BaseModel):
    """Output schema for meteorology analysis."""
    analysis: str = Field(..., description="Comprehensive meteorology analysis and insights")
...
class MeteorologyAgent(BaseAgent):
    """
    Specialized AI agent for meteorology.
    Expert AI for weather forecasting, atmospheric dynamics, and climate systems.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Meteorology Specialist",
            description="""Expert AI for weather forecasting, atmospheric dynamics, and climate systems.
            Specializes in weather forecasting, atmospheric dynamics, severe weather, climate modeling.
            ...
            price_per_use=2.0,
            tags=["meteorology", "meteorology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive meteorology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return MeteorologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return MeteorologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute meteorology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MeteorologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Meteorology Specialist with deep knowledge in weather forecasting, atmospheric dynamics, severe weather, climate modeling.

Your role is to:
1. Provide comprehensive analysis of meteorology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MeteorologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: MeteorologyInput
    ) -> MeteorologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard meteorology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "meteorology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["meteorology"]
        }
