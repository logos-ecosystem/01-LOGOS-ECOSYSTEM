"""
Atmospheric Sciences Expert - Expert in expert ai for weather systems, climate science, and atmospheric physics
"""
...
class AtmosphericSciencesAgentInput(BaseModel):
    """Input schema for atmospheric sciences queries."""
    query: str = Field(..., description="Atmospheric Sciences related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within atmospheric sciences")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class AtmosphericSciencesAgentOutput(BaseModel):
    """Output schema for atmospheric sciences analysis."""
    analysis: str = Field(..., description="Comprehensive atmospheric sciences analysis and insights")
...
class AtmosphericSciencesAgent(BaseAgent):
    """
    Specialized AI agent for atmospheric sciences.
    Expert AI for weather systems, climate science, and atmospheric physics.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Atmospheric Sciences Expert",
            description="""Expert AI for weather systems, climate science, and atmospheric physics.
            Specializes in weather prediction, climate modeling, atmospheric chemistry, meteorology.
            ...
            price_per_use=2.0,
            tags=["atmospheric-sciences", "atmospheric sciences", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive atmospheric sciences analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return AtmosphericSciencesAgentInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return AtmosphericSciencesAgentOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute atmospheric sciences analysis using Claude AI."""
        try:
            # Validate input
            validated_input = AtmosphericSciencesAgentInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Atmospheric Sciences Expert with deep knowledge in weather prediction, climate modeling, atmospheric chemistry, meteorology.

Your role is to:
1. Provide comprehensive analysis of atmospheric sciences queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: AtmosphericSciencesAgentInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: AtmosphericSciencesAgentInput
    ) -> AtmosphericSciencesAgentOutput:
        ...
        if not methodologies:
            methodologies = ["Standard atmospheric sciences analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "atmospheric sciences",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["atmospheric sciences"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in atmospheric sciences",
