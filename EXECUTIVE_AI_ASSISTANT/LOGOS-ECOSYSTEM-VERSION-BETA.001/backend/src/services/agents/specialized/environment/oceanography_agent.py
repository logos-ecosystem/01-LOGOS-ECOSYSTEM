"""
Oceanography Specialist - Expert in expert ai for ocean dynamics, marine systems, and ocean-atmosphere interactions
"""
...
class OceanographyInput(BaseModel):
    """Input schema for oceanography queries."""
    query: str = Field(..., description="Oceanography related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within oceanography")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class OceanographyOutput(BaseModel):
    """Output schema for oceanography analysis."""
    analysis: str = Field(..., description="Comprehensive oceanography analysis and insights")
...
class OceanographyAgent(BaseAgent):
    """
    Specialized AI agent for oceanography.
    Expert AI for ocean dynamics, marine systems, and ocean-atmosphere interactions.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Oceanography Specialist",
            description="""Expert AI for ocean dynamics, marine systems, and ocean-atmosphere interactions.
            Specializes in physical oceanography, marine ecosystems, ocean circulation, marine chemistry.
            ...
            price_per_use=2.0,
            tags=["oceanography", "oceanography", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive oceanography analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return OceanographyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return OceanographyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute oceanography analysis using Claude AI."""
        try:
            # Validate input
            validated_input = OceanographyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Oceanography Specialist with deep knowledge in physical oceanography, marine ecosystems, ocean circulation, marine chemistry.

Your role is to:
1. Provide comprehensive analysis of oceanography queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: OceanographyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: OceanographyInput
    ) -> OceanographyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard oceanography analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "oceanography",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["oceanography"]
        }
