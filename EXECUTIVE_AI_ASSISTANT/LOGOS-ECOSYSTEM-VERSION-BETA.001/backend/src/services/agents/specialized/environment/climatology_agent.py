"""
Climatology Expert - Expert in expert ai for climate systems, climate change, and paleoclimatology
"""
...
class ClimatologyInput(BaseModel):
    """Input schema for climatology queries."""
    query: str = Field(..., description="Climatology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within climatology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class ClimatologyOutput(BaseModel):
    """Output schema for climatology analysis."""
    analysis: str = Field(..., description="Comprehensive climatology analysis and insights")
...
class ClimatologyAgent(BaseAgent):
    """
    Specialized AI agent for climatology.
    Expert AI for climate systems, climate change, and paleoclimatology.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Climatology Expert",
            description="""Expert AI for climate systems, climate change, and paleoclimatology.
            Specializes in climate dynamics, climate modeling, paleoclimate, climate change impacts.
            ...
            price_per_use=2.0,
            tags=["climatology", "climatology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive climatology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return ClimatologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return ClimatologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute climatology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ClimatologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Climatology Expert with deep knowledge in climate dynamics, climate modeling, paleoclimate, climate change impacts.

Your role is to:
1. Provide comprehensive analysis of climatology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ClimatologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ClimatologyInput
    ) -> ClimatologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard climatology analysis", "Best practices evaluation", "Evidence-based approach"]
        ...
        technical_details = {
            ...
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["climatology"]
        }
