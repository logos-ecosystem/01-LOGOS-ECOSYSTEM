"""
Seismology Expert - Expert in expert ai for earthquakes, seismic waves, and earth's internal structure
"""
...
class SeismologyInput(BaseModel):
    """Input schema for seismology queries."""
    query: str = Field(..., description="Seismology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within seismology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class SeismologyOutput(BaseModel):
    """Output schema for seismology analysis."""
    analysis: str = Field(..., description="Comprehensive seismology analysis and insights")
...
class SeismologyAgent(BaseAgent):
    """
    Specialized AI agent for seismology.
    Expert AI for earthquakes, seismic waves, and Earth's internal structure.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Seismology Expert",
            description="""Expert AI for earthquakes, seismic waves, and Earth's internal structure.
            Specializes in earthquake mechanics, seismic wave propagation, hazard assessment, tectonics.
            ...
            price_per_use=2.0,
            tags=["seismology", "seismology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive seismology analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return SeismologyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return SeismologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute seismology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = SeismologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Seismology Expert with deep knowledge in earthquake mechanics, seismic wave propagation, hazard assessment, tectonics.

Your role is to:
1. Provide comprehensive analysis of seismology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: SeismologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: SeismologyInput
    ) -> SeismologyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard seismology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "seismology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["seismology"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in seismology",
