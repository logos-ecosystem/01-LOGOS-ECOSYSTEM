"""
Geology Specialist - Expert in expert ai for earth materials, geological processes, and earth history
"""
...
class GeologyInput(BaseModel):
    """Input schema for geology queries."""
    query: str = Field(..., description="Geology related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within geology")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class GeologyOutput(BaseModel):
    """Output schema for geology analysis."""
    analysis: str = Field(..., description="Comprehensive geology analysis and insights")
...
class GeologyAgent(BaseAgent):
    """
    Specialized AI agent for geology.
    Expert AI for Earth materials, geological processes, and Earth history.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Geology Specialist",
            description="""Expert AI for Earth materials, geological processes, and Earth history.
            Specializes in mineralogy, petrology, structural geology, stratigraphy, geological mapping.
            ...
            price_per_use=1.5,
            tags=["geology", "geology", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive geology analysis",
    ...
    def get_output_schema(self) -> Type[BaseModel]:
        ...
        return GeologyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute geology analysis using Claude AI."""
        try:
            # Validate input
            validated_input = GeologyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Geology Specialist with deep knowledge in mineralogy, petrology, structural geology, stratigraphy, geological mapping.

Your role is to:
1. Provide comprehensive analysis of geology queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: GeologyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        ...
        if not methodologies:
            methodologies = ["Standard geology analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "geology",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["geology"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in geology",
