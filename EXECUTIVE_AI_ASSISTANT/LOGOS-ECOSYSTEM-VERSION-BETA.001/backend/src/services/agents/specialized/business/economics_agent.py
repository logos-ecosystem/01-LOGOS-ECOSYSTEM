"""
Economics Specialist - Expert in expert ai for economic analysis, market dynamics, and economic policy
"""
...
class EconomicsInput(BaseModel):
    """Input schema for economics queries."""
    query: str = Field(..., description="Economics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within economics")
...
class EconomicsOutput(BaseModel):
    """Output schema for economics analysis."""
    analysis: str = Field(..., description="Comprehensive economics analysis and insights")
...
class EconomicsAgent(BaseAgent):
    """
    Specialized AI agent for economics.
    Expert AI for economic analysis, market dynamics, and economic policy.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Economics Specialist",
            description="""Expert AI for economic analysis, market dynamics, and economic policy.
            Specializes in microeconomics, macroeconomics, econometrics, market analysis, economic policy.
            ...
            price_per_use=2.0,
            tags=["economics", "economics", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute economics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = EconomicsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Economics Specialist with deep knowledge in microeconomics, macroeconomics, econometrics, market analysis, economic policy.

Your role is to:
1. Provide comprehensive analysis of economics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: EconomicsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: EconomicsInput
    ) -> EconomicsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard economics analysis", "Best practices evaluation", "Evidence-based approach"]
        ...
        technical_details = {
            ...
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["economics"]
        }
