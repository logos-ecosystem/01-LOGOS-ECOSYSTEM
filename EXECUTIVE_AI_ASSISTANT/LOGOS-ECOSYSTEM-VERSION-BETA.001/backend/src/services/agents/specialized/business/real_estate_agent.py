"""
Real Estate Specialist - Expert in expert ai for property valuation, real estate investment, and market analysis
"""
...
class RealEstateInput(BaseModel):
    """Input schema for real estate queries."""
    query: str = Field(..., description="Real Estate related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within real estate")
...
class RealEstateOutput(BaseModel):
    """Output schema for real estate analysis."""
    analysis: str = Field(..., description="Comprehensive real estate analysis and insights")
...
class RealEstateAgent(BaseAgent):
    """
    Specialized AI agent for real estate.
    Expert AI for property valuation, real estate investment, and market analysis.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Real Estate Specialist",
            description="""Expert AI for property valuation, real estate investment, and market analysis.
            Specializes in property valuation, market analysis, investment strategies, property management.
            ...
            price_per_use=2.0,
            tags=["real-estate", "real estate", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute real estate analysis using Claude AI."""
        try:
            # Validate input
            validated_input = RealEstateInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Real Estate Specialist with deep knowledge in property valuation, market analysis, investment strategies, property management.

Your role is to:
1. Provide comprehensive analysis of real estate queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: RealEstateInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: RealEstateInput
    ) -> RealEstateOutput:
        ...
        if not methodologies:
            methodologies = ["Standard real estate analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "real estate",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["real estate"]
        }
