"""
Social Media Strategy Specialist - Expert in expert ai for social media marketing, content strategy, and engagement optimization
"""
...
class SocialMediaStrategyInput(BaseModel):
    """Input schema for social media strategy queries."""
    query: str = Field(..., description="Social Media Strategy related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within social media strategy")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class SocialMediaStrategyOutput(BaseModel):
    """Output schema for social media strategy analysis."""
    analysis: str = Field(..., description="Comprehensive social media strategy analysis and insights")
...
class SocialMediaStrategyAgent(BaseAgent):
    """
    Specialized AI agent for social media strategy.
    Expert AI for social media marketing, content strategy, and engagement optimization.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Social Media Strategy Specialist",
            description="""Expert AI for social media marketing, content strategy, and engagement optimization.
            Specializes in content strategy, platform optimization, influencer marketing, social analytics.
            ...
            price_per_use=1.5,
            tags=["social-media-strategy", "social media strategy", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive social media strategy analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return SocialMediaStrategyInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return SocialMediaStrategyOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute social media strategy analysis using Claude AI."""
        try:
            # Validate input
            validated_input = SocialMediaStrategyInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Social Media Strategy Specialist with deep knowledge in content strategy, platform optimization, influencer marketing, social analytics.

Your role is to:
1. Provide comprehensive analysis of social media strategy queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: SocialMediaStrategyInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: SocialMediaStrategyInput
    ) -> SocialMediaStrategyOutput:
        ...
        if not methodologies:
            methodologies = ["Standard social media strategy analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "social media strategy",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["social media strategy"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in social media strategy",
