"""
Marketing Analytics Expert - Expert in expert ai for marketing data analysis, campaign optimization, and roi measurement
"""
...
class MarketingAnalyticsInput(BaseModel):
    """Input schema for marketing analytics queries."""
    query: str = Field(..., description="Marketing Analytics related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within marketing analytics")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MarketingAnalyticsOutput(BaseModel):
    """Output schema for marketing analytics analysis."""
    analysis: str = Field(..., description="Comprehensive marketing analytics analysis and insights")
...
class MarketingAnalyticsAgent(BaseAgent):
    """
    Specialized AI agent for marketing analytics.
    Expert AI for marketing data analysis, campaign optimization, and ROI measurement.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Marketing Analytics Expert",
            description="""Expert AI for marketing data analysis, campaign optimization, and ROI measurement.
            Specializes in customer analytics, campaign analysis, attribution modeling, market segmentation.
            ...
            price_per_use=2.0,
            tags=["marketing-analytics", "marketing analytics", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive marketing analytics analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return MarketingAnalyticsInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return MarketingAnalyticsOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute marketing analytics analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MarketingAnalyticsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Marketing Analytics Expert with deep knowledge in customer analytics, campaign analysis, attribution modeling, market segmentation.

Your role is to:
1. Provide comprehensive analysis of marketing analytics queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MarketingAnalyticsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: MarketingAnalyticsInput
    ) -> MarketingAnalyticsOutput:
        ...
        if not methodologies:
            methodologies = ["Standard marketing analytics analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "marketing analytics",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["marketing analytics"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in marketing analytics",
