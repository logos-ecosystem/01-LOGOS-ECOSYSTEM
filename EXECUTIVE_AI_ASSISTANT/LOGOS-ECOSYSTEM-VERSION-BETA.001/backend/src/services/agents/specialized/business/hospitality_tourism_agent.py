"""
Hospitality & Tourism Specialist - Expert in expert ai for hospitality management, tourism development, and guest services
"""
...
class HospitalityTourismInput(BaseModel):
    """Input schema for hospitality tourism queries."""
    query: str = Field(..., description="Hospitality Tourism related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within hospitality tourism")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class HospitalityTourismOutput(BaseModel):
    """Output schema for hospitality tourism analysis."""
    analysis: str = Field(..., description="Comprehensive hospitality tourism analysis and insights")
...
class HospitalityTourismAgent(BaseAgent):
    """
    Specialized AI agent for hospitality tourism.
    Expert AI for hospitality management, tourism development, and guest services.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Hospitality & Tourism Specialist",
            description="""Expert AI for hospitality management, tourism development, and guest services.
            Specializes in hotel management, tourism marketing, guest experience, revenue management.
            ...
            price_per_use=1.5,
            tags=["hospitality-tourism", "hospitality tourism", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive hospitality tourism analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return HospitalityTourismInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return HospitalityTourismOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute hospitality tourism analysis using Claude AI."""
        try:
            # Validate input
            validated_input = HospitalityTourismInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Hospitality & Tourism Specialist with deep knowledge in hotel management, tourism marketing, guest experience, revenue management.

Your role is to:
1. Provide comprehensive analysis of hospitality tourism queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: HospitalityTourismInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: HospitalityTourismInput
    ) -> HospitalityTourismOutput:
        ...
        if not methodologies:
            methodologies = ["Standard hospitality tourism analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "hospitality tourism",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["hospitality tourism"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in hospitality tourism",
