"""
Manufacturing Expert - Expert in expert ai for manufacturing processes, production optimization, and quality control
"""
...
class ManufacturingInput(BaseModel):
    """Input schema for manufacturing queries."""
    query: str = Field(..., description="Manufacturing related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within manufacturing")
...
class ManufacturingOutput(BaseModel):
    """Output schema for manufacturing analysis."""
    analysis: str = Field(..., description="Comprehensive manufacturing analysis and insights")
...
class ManufacturingAgent(BaseAgent):
    """
    Specialized AI agent for manufacturing.
    Expert AI for manufacturing processes, production optimization, and quality control.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Manufacturing Expert",
            description="""Expert AI for manufacturing processes, production optimization, and quality control.
            Specializes in lean manufacturing, quality control, supply chain, automation, process optimization.
            ...
            price_per_use=2.0,
            tags=["manufacturing", "manufacturing", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute manufacturing analysis using Claude AI."""
        try:
            # Validate input
            validated_input = ManufacturingInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Manufacturing Expert with deep knowledge in lean manufacturing, quality control, supply chain, automation, process optimization.

Your role is to:
1. Provide comprehensive analysis of manufacturing queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: ManufacturingInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: ManufacturingInput
    ) -> ManufacturingOutput:
        ...
        if not methodologies:
            methodologies = ["Standard manufacturing analysis", "Best practices evaluation", "Evidence-based approach"]
        ...
        technical_details = {
            ...
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["manufacturing"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in manufacturing",
