"""
Theater & Drama Expert - Expert in expert ai for theatrical arts, dramatic analysis, and performance techniques
"""
...
class TheaterDramaInput(BaseModel):
    """Input schema for theater drama queries."""
    query: str = Field(..., description="Theater Drama related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within theater drama")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class TheaterDramaOutput(BaseModel):
    """Output schema for theater drama analysis."""
    analysis: str = Field(..., description="Comprehensive theater drama analysis and insights")
...
class TheaterDramaAgent(BaseAgent):
    """
    Specialized AI agent for theater drama.
    Expert AI for theatrical arts, dramatic analysis, and performance techniques.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Theater & Drama Expert",
            description="""Expert AI for theatrical arts, dramatic analysis, and performance techniques.
            Specializes in acting techniques, directing, playwriting, theater history, stagecraft.
            ...
            price_per_use=1.5,
            tags=["theater-drama", "theater drama", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive theater drama analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return TheaterDramaInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return TheaterDramaOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute theater drama analysis using Claude AI."""
        try:
            # Validate input
            validated_input = TheaterDramaInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Theater & Drama Expert with deep knowledge in acting techniques, directing, playwriting, theater history, stagecraft.

Your role is to:
1. Provide comprehensive analysis of theater drama queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: TheaterDramaInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: TheaterDramaInput
    ) -> TheaterDramaOutput:
        ...
        if not methodologies:
            methodologies = ["Standard theater drama analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "theater drama",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["theater drama"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in theater drama",
