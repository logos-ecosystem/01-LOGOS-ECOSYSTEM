"""
Music Performance Specialist - Expert in expert ai for musical performance, technique, and interpretation
"""
...
class MusicPerformanceInput(BaseModel):
    """Input schema for music performance queries."""
    query: str = Field(..., description="Music Performance related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within music performance")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MusicPerformanceOutput(BaseModel):
    """Output schema for music performance analysis."""
    analysis: str = Field(..., description="Comprehensive music performance analysis and insights")
...
class MusicPerformanceAgent(BaseAgent):
    """
    Specialized AI agent for music performance.
    Expert AI for musical performance, technique, and interpretation.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Music Performance Specialist",
            description="""Expert AI for musical performance, technique, and interpretation.
            Specializes in instrumental technique, vocal performance, ensemble playing, music interpretation.
            ...
            price_per_use=1.5,
            tags=["music-performance", "music performance", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive music performance analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return MusicPerformanceInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return MusicPerformanceOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute music performance analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MusicPerformanceInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Music Performance Specialist with deep knowledge in instrumental technique, vocal performance, ensemble playing, music interpretation.

Your role is to:
1. Provide comprehensive analysis of music performance queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MusicPerformanceInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: MusicPerformanceInput
    ) -> MusicPerformanceOutput:
        ...
        if not methodologies:
            methodologies = ["Standard music performance analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "music performance",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["music performance"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in music performance",
