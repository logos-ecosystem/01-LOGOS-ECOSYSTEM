"""
Dance Arts Specialist - Expert in expert ai for dance techniques, choreography, and movement analysis
"""
...
class DanceInput(BaseModel):
    """Input schema for dance queries."""
    query: str = Field(..., description="Dance related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within dance")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class DanceOutput(BaseModel):
    """Output schema for dance analysis."""
    analysis: str = Field(..., description="Comprehensive dance analysis and insights")
...
class DanceAgent(BaseAgent):
    """
    Specialized AI agent for dance.
    Expert AI for dance techniques, choreography, and movement analysis.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Dance Arts Specialist",
            description="""Expert AI for dance techniques, choreography, and movement analysis.
            Specializes in ballet, contemporary, hip-hop, choreography, dance history.
            ...
            price_per_use=1.5,
            tags=["dance", "dance", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive dance analysis",
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute dance analysis using Claude AI."""
        try:
            # Validate input
            validated_input = DanceInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Dance Arts Specialist with deep knowledge in ballet, contemporary, hip-hop, choreography, dance history.

Your role is to:
1. Provide comprehensive analysis of dance queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: DanceInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        ...
        if not methodologies:
            methodologies = ["Standard dance analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "dance",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["dance"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in dance",
