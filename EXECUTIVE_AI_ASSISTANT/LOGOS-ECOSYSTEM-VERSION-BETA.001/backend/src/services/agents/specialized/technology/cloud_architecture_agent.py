"""
Cloud Architecture Specialist - Expert in expert ai for cloud design patterns, scalability, and cloud-native applications
"""
...
class CloudArchitectureInput(BaseModel):
    """Input schema for cloud architecture queries."""
    query: str = Field(..., description="Cloud Architecture related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within cloud architecture")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class CloudArchitectureOutput(BaseModel):
    """Output schema for cloud architecture analysis."""
    analysis: str = Field(..., description="Comprehensive cloud architecture analysis and insights")
...
class CloudArchitectureAgent(BaseAgent):
    """
    Specialized AI agent for cloud architecture.
    Expert AI for cloud design patterns, scalability, and cloud-native applications.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="Cloud Architecture Specialist",
            description="""Expert AI for cloud design patterns, scalability, and cloud-native applications.
            Specializes in AWS/Azure/GCP, microservices, serverless, cloud security, cost optimization.
            ...
            price_per_use=2.0,
            tags=["cloud-architecture", "cloud architecture", "expert", "analysis", "consulting"],
            capabilities=[
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return CloudArchitectureInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return CloudArchitectureOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute cloud architecture analysis using Claude AI."""
        try:
            # Validate input
            validated_input = CloudArchitectureInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert Cloud Architecture Specialist with deep knowledge in AWS/Azure/GCP, microservices, serverless, cloud security, cost optimization.

Your role is to:
1. Provide comprehensive analysis of cloud architecture queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: CloudArchitectureInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: CloudArchitectureInput
    ) -> CloudArchitectureOutput:
        ...
        if not methodologies:
            methodologies = ["Standard cloud architecture analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "cloud architecture",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["cloud architecture"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in cloud architecture",
