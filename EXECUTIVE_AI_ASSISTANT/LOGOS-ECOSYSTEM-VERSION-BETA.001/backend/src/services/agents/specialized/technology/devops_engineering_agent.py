"""
DevOps Engineering Expert - Expert in expert ai for CI/CD pipelines, infrastructure automation, and deployment strategies
"""
...
class DevopsEngineeringAgentInput(BaseModel):
    """Input schema for devops engineering queries."""
    query: str = Field(..., description="DevOps Engineering related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within devops engineering")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class DevopsEngineeringAgentOutput(BaseModel):
    """Output schema for devops engineering analysis."""
    analysis: str = Field(..., description="Comprehensive devops engineering analysis and insights")
...
class DevopsEngineeringAgent(BaseAgent):
    """
    Specialized AI agent for devops engineering.
    Expert AI for CI/CD pipelines, infrastructure automation, and deployment strategies.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="DevOps Engineering Expert",
            description="""Expert AI for CI/CD pipelines, infrastructure automation, and deployment strategies.
            Specializes in continuous integration, deployment automation, infrastructure as code, monitoring.
            ...
            price_per_use=2.5,
            tags=["devops-engineering", "devops engineering", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive devops engineering analysis",
    ...
    def get_input_schema(self) -> Type[BaseModel]:
        ...
        return DevopsEngineeringAgentInput
    
    def get_output_schema(self) -> Type[BaseModel]:
        """Get the output schema for this agent."""
        return DevopsEngineeringAgentOutput
    
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute devops engineering analysis using Claude AI."""
        try:
            # Validate input
            validated_input = DevopsEngineeringAgentInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert DevOps Engineering Expert with deep knowledge in continuous integration, deployment automation, infrastructure as code, monitoring.

Your role is to:
1. Provide comprehensive analysis of devops engineering queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: DevopsEngineeringAgentInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        self, 
        ai_response: str, 
        validated_input: DevopsEngineeringAgentInput
    ) -> DevopsEngineeringAgentOutput:
        ...
        if not methodologies:
            methodologies = ["Standard devops engineering analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "devops engineering",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["devops engineering"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in devops engineering",
