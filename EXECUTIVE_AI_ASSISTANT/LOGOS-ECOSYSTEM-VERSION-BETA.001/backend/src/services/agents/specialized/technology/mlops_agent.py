"""
MLOps Expert - Expert in expert ai for ml operations, model lifecycle management, and production ml
"""
...
class MlopsInput(BaseModel):
    """Input schema for mlops queries."""
    query: str = Field(..., description="Mlops related question or request")
    ...
    focus_area: Optional[str] = Field(None, description="Specific area of focus within mlops")
    detail_level: Optional[str] = Field("comprehensive", description="Level of detail required (basic/intermediate/comprehensive)")


class MlopsOutput(BaseModel):
    """Output schema for mlops analysis."""
    analysis: str = Field(..., description="Comprehensive mlops analysis and insights")
...
class MlopsAgent(BaseAgent):
    """
    Specialized AI agent for mlops.
    Expert AI for ML operations, model lifecycle management, and production ML.
    """
    
    def __init__(self):
        metadata = AgentMetadata(
            name="MLOps Expert",
            description="""Expert AI for ML operations, model lifecycle management, and production ML.
            Specializes in model versioning, experiment tracking, model monitoring, deployment pipelines.
            ...
            price_per_use=2.5,
            tags=["mlops", "mlops", "expert", "analysis", "consulting"],
            capabilities=[
                "Comprehensive mlops analysis",
    ...
    async def _execute(self, input_data: AgentInput) -> AgentOutput:
        """Execute mlops analysis using Claude AI."""
        try:
            # Validate input
            validated_input = MlopsInput(**input_data.input_data)
            
            # Create specialized system prompt
            system_prompt = """You are an expert MLOps Expert with deep knowledge in model versioning, experiment tracking, model monitoring, deployment pipelines.

Your role is to:
1. Provide comprehensive analysis of mlops queries
...
- Clear and well-organized"""
    ...
    async def _build_prompt(self, validated_input: MlopsInput) -> str:
...
6. References and further resources""")
    ...
    async def _parse_response(
        ...
        if not methodologies:
            methodologies = ["Standard mlops analysis", "Best practices evaluation", "Evidence-based approach"]
        
        # Technical details (placeholder - would extract from response in production)
        technical_details = {
            "analysis_type": "mlops",
            "complexity_level": validated_input.detail_level,
            "focus_areas": [validated_input.focus_area] if validated_input.focus_area else ["mlops"]
        }
        
        # References
        references = [
            "Industry best practices and standards",
            "Current research in mlops",
